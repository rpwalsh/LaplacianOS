...
fn main() {
    ...
    let workspace_root = manifest_dir.parent().expect("LaplacianOS workspace root");
    ...
    println!(
        "cargo:rustc-env=LAPLACIANOS_BUILD_VERSION={}",
        env::var("CARGO_PKG_VERSION").expect("package version")
    );
    println!("cargo:rustc-env=LAPLACIANOS_BUILD_PROFILE={profile}");
    if git_dir.is_dir() {
    ...
    println!("cargo:rustc-env=LAPLACIANOS_BUILD_GIT_SHA={git_commit}");
    println!("cargo:rustc-env=LAPLACIANOS_BUILD_GIT_DIRTY={git_dirty}");
    ...
    let protected_target = match target_arch.as_str() {
        "x86_64" => "x86_64-unknown-none",
        "aarch64" => "aarch64-unknown-none",
        other => panic!("unsupported LaplacianOS protected-service architecture: {other}"),
    };
    if target != protected_target {
        panic!(
            "kernel target {target} does not match protected-service target {protected_target}"
        );
    }

    build_protected_services(
        &protected_dir,
        &protected_target_dir,
        protected_profile,
        protected_target,
    );

    let profile_dir = protected_target_dir
        .join(protected_target)
        .join(protected_profile);
    let package_store = protected_target_dir.join("laposp.pkg");
    let package_entry_count = build_package_store(&protected_dir, &profile_dir, &package_store);
    println!("cargo:rustc-env=LAPLACIANOS_PACKAGE_STORE_ENTRY_COUNT={package_entry_count}");
    println!("cargo:rustc-env=LAPLACIANOS_PACKAGE_STORE_FORMAT=2");
    println!(
        "cargo:rustc-env=LAPLACIANOS_PACKAGE_STORE={}",
        package_store.display()
    );
    for (key, bin) in [
        ("LAPLACIANOS_RING3_INIT_ELF", "init"),
        ("LAPLACIANOS_RING3_SERVICEMGR_ELF", "servicemgr"),
        ("LAPLACIANOS_RING3_GRAPHD_ELF", "graphd"),
        ("LAPLACIANOS_RING3_MODELD_ELF", "modeld"),
        ("LAPLACIANOS_RING3_TRAINERD_ELF", "trainerd"),
        ("LAPLACIANOS_RING3_ARTIFACTSD_ELF", "artifactsd"),
        ("LAPLACIANOS_RING3_SYSD_ELF", "sysd"),
        ("LAPLACIANOS_RING3_COMPOSITOR_ELF", "compositor"),
        ("LAPLACIANOS_RING3_SSHD_ELF", "sshd"),
        ("LAPLACIANOS_RING3_NETD_ELF", "netd"),
    ] {
...
fn build_protected_services(
    ...
    let protected_rustflags = if target.starts_with("x86_64") {
        "-C relocation-model=pic -C code-model=large -C force-frame-pointers=yes"
    } else {
        "-C relocation-model=pic -C force-frame-pointers=yes"
    };
    ...
    command.env(target_rustflags, protected_rustflags);
    if profile == "release" {
        command.arg("--release");
    }
    let status = command
