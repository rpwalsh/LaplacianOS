...
fn run_qemu_serial_oracle(config: OracleConfig) {
    let Some(command_line) = std::env::var("LAPLACIANOS_E2E_CMD").ok() else {
        eprintln!("skipping: LAPLACIANOS_E2E_CMD is not set");
        return;
    };

    let mut parts = command_line.split_whitespace();
    let Some(program) = parts.next() else {
        panic!("LAPLACIANOS_E2E_CMD is empty");
    };
...
fn e2e_boot_reaches_ready_marker_without_panic() {
    let ready_marker = std::env::var("LAPLACIANOS_E2E_READY_MARKER")
        .unwrap_or_else(|_| "[desktop] host loop started".to_string());
    let fail_markers = parse_csv_env_or("panic,[panic],FAIL", "LAPLACIANOS_E2E_FAIL_MARKERS");
    let timeout_secs = std::env::var("LAPLACIANOS_E2E_TIMEOUT_SECS")
        .ok()
