...
/// Compiler stack-protector failure hook shared by every supported ISA.
...
fn try_write_crash_dump(info: &PanicInfo) {
    ...
    let _ = crate::vfs::write(fd, b"LAPLACIANOS_CRASH_DUMP_V1\n");
    let _ = crate::vfs::write(fd, b"path: ");
    let _ = crate::vfs::write(fd, path);
    let _ = crate::vfs::write(fd, b"\n");
    let _ = crate::vfs::write(fd, b"build: v");
    let _ = crate::vfs::write(fd, env!("LAPLACIANOS_BUILD_VERSION").as_bytes());
    let _ = crate::vfs::write(fd, b" commit=");
    let _ = crate::vfs::write(fd, env!("LAPLACIANOS_BUILD_GIT_SHA").as_bytes());
    let _ = crate::vfs::write(fd, b"\n");
