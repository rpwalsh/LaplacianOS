...
mod tests {
    ...
    /// 4. Assert success
    #[test]
    fn test_laplacianos_release_bundle_signature() {
        // Example: Signature of a LaplacianOS release bundle
        // Generated via: laplacianos-release-sign --input laplacianos-v1.0.0.iso --output laplacianos-v1.0.0.iso.sig
...
fn main() {
    ...
    //    let message = std::fs::read("laplacianos-v1.0.0.iso")?;
    //    let signature = keypair.sign(&message);
    //    std::fs::write("laplacianos-v1.0.0.iso.sig", signature.to_bytes())?;
    //    ```
    //
...
//    $ hsm keygen --algo ed25519 --label laplacianos-release-v1 --export-pub
//    Public Key: d75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511e
//
...
//    → Produces laplacianos-v1.0.0.iso (2.2 GB)
//
// 2. Sign each artifact using HSM:
//    $ hsm sign --key-label laplacianos-release-v1 \
//               --input laplacianos-v1.0.0.iso \
//               --output laplacianos-v1.0.0.iso.sig
//    → Produces .sig file (64 bytes, ed25519 signature)
//
// 3. Verify signature on the signing station using ed25519-dalek:
//    $ laplacianos-verify --pubkey docs/release-key.pub \
//                     --input laplacianos-v1.0.0.iso \
//                     --sig laplacianos-v1.0.0.iso.sig
...
//
// 2. Before loading LAPOSP.PKG, kernel verifies:
...
mod dalek_interop {
    ...
    fn test_sign_and_verify_round_trip() {
        ...
        let message = b"LaplacianOS release artifact";
