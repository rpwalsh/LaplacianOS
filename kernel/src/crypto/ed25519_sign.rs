...
//!
//! All field arithmetic is a copy of the TweetNaCl 16-limb GF(2²⁵⁵−19)
//! representation used in the verifier so that the two modules remain
//! independent and each can be audited in isolation.
//!
//! # Key representation
//! * `seed`           — 32 random bytes (private)
//! * `public_key`     — 32 bytes (compressed Edwards y-coordinate with sign bit)
//! * `expanded_secret`— 64 bytes = SHA-512(seed); first 32 bytes clamped as the
...
// ── Group order L and scalar arithmetic ────────────────────────────────────────
//
...
fn mod_l(s: &[u8; 64]) -> [u8; 32] {
    // Work in i64 to avoid overflow — each limb is at most 21 bits wide.
    let mut t = [0i64; 64];
    for i in 0..64 {
        t[i] = s[i] as i64;
    }
...
///
/// Used to compute S = r + H(R||A||M) · a  in Ed25519.
fn scalar_muladd(a: &[u8; 32], b: &[u8; 32], c: &[u8; 32]) -> [u8; 32] {
    // Multiply b*c using schoolbook 32×32 → 64 bytes.
    let mut bc = [0i64; 64];
    for i in 0..32 {
        for j in 0..32 {
...
/// Clamp the low 32 bytes of the expanded secret key per RFC 8032 §5.1.5.
