//! LaplacianOS Kernel — freestanding x86_64 entry point
//!
...
const BUILD_VERSION: &str = env!("LAPLACIANOS_BUILD_VERSION");
const BUILD_PROFILE: &str = env!("LAPLACIANOS_BUILD_PROFILE");
const BUILD_GIT_SHA: &str = env!("LAPLACIANOS_BUILD_GIT_SHA");
const BUILD_GIT_DIRTY: &str = env!("LAPLACIANOS_BUILD_GIT_DIRTY");
const PACKAGE_STORE_FORMAT: &str = env!("LAPLACIANOS_PACKAGE_STORE_FORMAT");
const PACKAGE_STORE_ENTRY_COUNT: &str = env!("LAPLACIANOS_PACKAGE_STORE_ENTRY_COUNT");
...
static ARM_PACKAGE_STORE: &[u8] = include_bytes!(env!("LAPLACIANOS_PACKAGE_STORE"));

#[cfg(target_arch = "aarch64")]
static mut ARM_MEMORY_REGIONS: [bootinfo::MemoryRegion; 1] = [bootinfo::MemoryRegion {
    start: 0,
    length: 0,
    kind: bootinfo::MemoryRegionKind::Unknown,
    _pad: 0,
}];

#[cfg(target_arch = "aarch64")]
static mut ARM_BOOT_INFO: BootInfo = BootInfo {
    ...
    framebuffer_format: bootinfo::FramebufferFormat::Unknown,
    ...
    kernel_phys_end: 0,
    boot_modules_ptr: 0,
    boot_modules_count: 0,
    package_store_ptr: 0,
    package_store_size: 0,
};
...
pub extern "C" fn laplacianos_kmain(boot_info: &'static BootInfo) -> ! {
    // ================================================================
    ...
    crate::arch::serial::write_line(b"========================================");
    crate::arch::serial::write_line(b"  LaplacianOS Kernel - early boot");
    crate::arch::serial::write_line(b"========================================");
    ...
    // ================================================================
    // Stage 10: Graph subsystem — the core abstraction of LaplacianOS
    // ================================================================
    ...
    crate::arch::serial::write_line(b"");
    crate::arch::serial::write_line(b"========================================");
    crate::arch::serial::write_line(b"  LaplacianOS boot complete - core stages OK");
    crate::arch::serial::write_line(b"  Telemetry scheduling: adaptive quantum");
...
fn log_release_identity() {
    crate::arch::serial::write_bytes(b"[boot] Release: LaplacianOS v");
...
fn kernel_init_task() {
    ...
    // Send a test message.
    let test_payload = b"laplacianos:init:hello";
    ...
    let echo_payload = b"laplacianos:echo:test";
    ...
    #[cfg(feature = "boot-demos")]
    {
        if test_failures == 0 {
            ...
            // By this point the facade has received observations from:
            ...
            // -- Scheduler hint diagnostic ------------------------------------
            // Query the facade for a scheduling hint to prove the public
            // telemetry path is wired end-to-end.
            {
            ...
            // Proves the review pipeline from indexing to retrieval.
...
pub(crate) fn reboot_laplacianos_from_desktop() -> ! {
    graceful_protected_shutdown();
    arch::machine::reboot()
}

pub(crate) fn shutdown_laplacianos_from_desktop() -> ! {
    graceful_protected_shutdown();
...
fn cognitive_smoke_test() {
    ...
    let test_doc = b"LaplacianOS is a graph-native operating system for local AI workloads. \
It uses spectral analysis and PageRank for knowledge retrieval. \
The kernel maintains a typed timestamped graph with provenance tracking.";
    ...
    bloom.insert(b"laplacianos");
    bloom.insert(b"kernel");
    bloom.insert(b"spectral");
    let bloom_hit = bloom.query(b"laplacianos");
    ...
    crate::arch::serial::write_bytes(b"[cognitive]        bloom: laplacianos=");
    if bloom_hit {
    ...
    let (pred_token, pred_prob) = match (kn.token_id(b"the"), kn.token_id(b"graph")) {
        (Some(first), Some(second)) => kn.predict(first, second),
        _ => (0, 0),
    };
    ...
    crate::arch::serial::write_u64_dec_inline(pr.channels_used as u64);
    ...
    crate::arch::serial::write_u64_dec_inline(pr.support_score as u64);
    ...
    crate::arch::serial::write_u64_dec(pr.provenance_live_nodes as u64);
...
const SUPERVISOR_READY_MASK: u8 = (1 << 0) | (1 << 1);
const REQUIRED_PROTECTED_READY_MASK: u8 = u8::MAX;
...
impl ProtectedBootstrapReport {
    ...
    fn critical_ready_count(self) -> u32 {
        (self.ready_mask & SUPERVISOR_READY_MASK).count_ones()
    }
    ...
    fn is_ready(self) -> bool {
        self.uinit_online
            ...
            && !self.degraded_seen
            && !self.failure_seen
            && self.ready_mask == REQUIRED_PROTECTED_READY_MASK
    }
    ...
    fn is_degraded_bootable(self) -> bool {
        self.uinit_online
            ...
            && self.ready_mask & SUPERVISOR_READY_MASK == SUPERVISOR_READY_MASK
    }
}
...
// ====================================================================
// Predictive telemetry runtime
// ====================================================================

/// Initialise the predictive telemetry runtime.
///
/// Registers hardware sensors as graph nodes and seeds the initial telemetry
/// cycle used by the predictive runtime.
fn twin_init() {
    use graph::twin::*;
    use graph::types::WEIGHT_ONE;

    crate::arch::serial::write_line(b"[twin] === Predictive Telemetry Runtime ===");
    ...
    // Provide baseline readings until live device telemetry supersedes them.
    ...
    crate::arch::serial::write_line(b"[twin] === Public Telemetry Online ===");
}
...
pub fn kmain_aarch64() -> ! {
    arch::aarch64::serial::write_line_raw(b"[boot] LaplacianOS AArch64 kernel");
    arch::aarch64::serial::write_line_raw(b"[boot] exception vectors online");
    let dtb = arch::aarch64::fdt::dtb_address();
    arch::aarch64::serial::write_bytes_raw(b"[boot] firmware DTB=");
    arch::aarch64::serial::write_hex_raw(dtb);
    if dtb != 0 {
        let first = unsafe { core::ptr::read_volatile(dtb as *const u64) };
        arch::aarch64::serial::write_bytes_raw(b" first8=");
        arch::aarch64::serial::write_hex_raw(first);
    }
    arch::aarch64::serial::write_bytes_raw(b"\r\n");
    if !arch::aarch64::fdt::is_valid() {
        arch::aarch64::serial::write_line_raw(b"[boot] FATAL: invalid firmware device tree");
        arch::aarch64::halt();
    }
    let Some(memory) = arch::aarch64::fdt::memory() else {
        arch::aarch64::serial::write_line_raw(b"[boot] FATAL: device tree contains no RAM region");
        arch::aarch64::halt();
    };
    arch::aarch64::serial::write_bytes_raw(b"[boot] DTB RAM base=");
    arch::aarch64::serial::write_hex_raw(memory.base);
    arch::aarch64::serial::write_bytes_raw(b" size=");
    arch::aarch64::serial::write_u64_dec_inline_raw(memory.size);
    arch::aarch64::serial::write_bytes_raw(b"\r\n[boot] DTB CPUs=");
    arch::aarch64::serial::write_u64_dec_inline_raw(arch::aarch64::fdt::cpu_count() as u64);
    arch::aarch64::serial::write_bytes_raw(b"\r\n[boot] DTB virtio-mmio transports=");
    arch::aarch64::serial::write_u64_dec_inline_raw(
        arch::aarch64::fdt::virtio_mmio_count() as u64,
    );
    arch::aarch64::serial::write_bytes_raw(b"\r\n");
    let boot_info = prepare_aarch64_boot_info(memory);
    users::init_defaults();
    log_release_identity();
    boot_progress(1, BOOT_PROGRESS_TOTAL, b"firmware discovery");

    let diagnostics = log_bootinfo(boot_info);
    if diagnostics.intersects(
        BootInfoDiag::VERSION_MISMATCH
            | BootInfoDiag::SIZE_MISMATCH
            | BootInfoDiag::NO_MEMORY_MAP,
    ) {
        crate::arch::serial::write_line(b"[boot] FATAL: invalid ARM boot contract");
        halt_loop();
    }
    mm::kaslr::init(boot_info.kernel_phys_start);
    arch::user::init_fast_syscalls(0);
    ui::desktop::init_screen(boot_info);

    mm::phys::init(boot_info);
    register_reserved_ranges(boot_info);
    let dtb_end = dtb.saturating_add(arch::aarch64::fdt::dtb_size());
    if dtb_end > dtb {
        unsafe { mm::reserved::add(dtb, dtb_end, b"firmware device tree") };
    }
    mm::frame_alloc::init();
    let Some(first_frame) = mm::frame_alloc::alloc_frame() else {
        crate::arch::serial::write_line(b"[boot] FATAL: ARM frame allocator has no usable RAM");
        halt_loop();
    };
    mm::frame_alloc::dealloc_frame(first_frame);
    mm::frame_alloc::log_summary();
    boot_progress(5, BOOT_PROGRESS_TOTAL, b"physical memory online");

    unsafe { mm::page_table::enable_nxe() };
    let identity_limit = memory.base.saturating_add(memory.size);
    let Some(page_tables) = (unsafe { mm::page_table::build_identity_map(identity_limit) }) else {
        crate::arch::serial::write_line(b"[boot] FATAL: ARM translation-table allocation failed");
        halt_loop();
    };
    let errors = unsafe { mm::page_table::validate(&page_tables) };
    if errors != 0 {
        crate::arch::serial::write_bytes(b"[boot] FATAL: ARM translation-table errors=");
        crate::arch::serial::write_u64_dec(errors as u64);
        halt_loop();
    }
    unsafe {
        mm::page_table::register_pt_frames_reserved();
        mm::page_table::activate(&page_tables);
        mm::page_table::remap_kernel_sections();
    }
    boot_progress(8, BOOT_PROGRESS_TOTAL, b"mmu and kernel W^X");

    arch::aarch64::virtio_mmio::log_discovery();
    let Some(entropy) = arch::entropy::random_u64() else {
        crate::arch::serial::write_line(b"[boot] FATAL: no hardware entropy source");
        halt_loop();
    };
    unsafe { crate::panic::__stack_chk_guard = entropy };
    net::tcp::seed_entropy(entropy.rotate_left(29) ^ arch::cpu::cycle_counter());

    let stack_count = task::stack_pool::init(BOOT_STACK_TARGET);
    if stack_count < BOOT_STACK_MIN_RESIDENT {
        crate::arch::serial::write_line(b"[boot] FATAL: insufficient ARM task stacks");
        halt_loop();
    }
    mm::heap::init();
    if mm::heap::stats().capacity == 0 {
        crate::arch::serial::write_line(b"[boot] FATAL: ARM kernel heap unavailable");
        halt_loop();
    }
    vfs::init(boot_info);
    arch::pci::log_scan();
    drivers::init();
    let driver_probe = drivers::probe_all();
    crate::arch::serial::write_bytes(b"[boot] ARM drivers bound=");
    crate::arch::serial::write_u64_dec_inline(driver_probe.bound as u64);
    crate::arch::serial::write_bytes(b" managed=");
    crate::arch::serial::write_u64_dec_inline(driver_probe.managed as u64);
    crate::arch::serial::write_bytes(b" failed=");
    crate::arch::serial::write_u64_dec(driver_probe.failed as u64);
    storage::block::init();

    let (display_width, display_height) = drivers::gpu::virtio_gpu::resolution();
    if display_width == 0 || display_height == 0 {
        crate::arch::serial::write_line(b"[boot] FATAL: ARM runtime scanout unavailable");
        halt_loop();
    }
    drivers::display::attach_runtime_framebuffer(
        drivers::gpu::virtio_gpu::framebuffer_addr(),
        display_width,
        display_height,
        display_width,
        bootinfo::FramebufferFormat::Bgr,
    );
    let _pointer_ready = input::pointer::init(display_width, display_height);
    boot_progress(11, BOOT_PROGRESS_TOTAL, b"vfs drivers display input");
    ...
    match task::table::create_kernel_task_with_index(b"init", init_entry) {
        Some((id, _)) => {
            crate::arch::serial::write_bytes(b"[boot] ARM init task id=");
            crate::arch::serial::write_u64_dec(id);
            graph::seed::register_task(b"init", graph::types::NODE_ID_KERNEL);
        }
        None => {
            crate::arch::serial::write_line(b"[boot] FATAL: ARM init task creation failed");
            halt_loop();
        }
    }
    if graph::arena::audit() != 0 {
        crate::arch::serial::write_line(b"[boot] FATAL: ARM graph integrity audit failed");
        halt_loop();
    }
    svc::init();
    sched::init();
    arch::timer::init();
    crate::arch::serial::write_line(b"[boot] LaplacianOS ARM64 core boot complete");
    boot_progress(15, BOOT_PROGRESS_TOTAL, b"dispatching init");
    arch::cpu::enable_interrupts();
    unsafe { sched::dispatch_first() };
    crate::arch::serial::write_line(b"[boot] ARM tasks complete");
    halt_loop()
}

#[cfg(target_arch = "aarch64")]
fn prepare_aarch64_boot_info(memory: arch::aarch64::fdt::Region) -> &'static BootInfo {
    let kernel_start = core::ptr::addr_of!(__kernel_start) as u64;
    let kernel_end = core::ptr::addr_of!(__kernel_end) as u64;
    unsafe {
        let regions = &raw mut ARM_MEMORY_REGIONS;
        (*regions)[0] = bootinfo::MemoryRegion {
            start: memory.base,
            length: memory.size,
            kind: bootinfo::MemoryRegionKind::Usable,
            _pad: 0,
        };
        let info = &raw mut ARM_BOOT_INFO;
        (*info).memory_regions_ptr = core::ptr::addr_of!(ARM_MEMORY_REGIONS) as u64;
        (*info).memory_regions_count = 1;
        (*info).kernel_phys_start = kernel_start;
        (*info).kernel_phys_end = kernel_end;
        (*info).package_store_ptr = ARM_PACKAGE_STORE.as_ptr() as u64;
        (*info).package_store_size = ARM_PACKAGE_STORE.len() as u64;
        &*(&raw const ARM_BOOT_INFO)
    }
}
