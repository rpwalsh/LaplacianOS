...
const VIRTIO_GPU_MMIO_DEVICE_ID: u32 = 16;
...
struct GpuState {
    ...
    io_base: u16,
    /// Virtio-MMIO register base used by firmware-described ARM systems.
    mmio_base: u64,
...
pub fn probe_driver() -> ProbeResult {
    #[cfg(target_arch = "aarch64")]
    if let Some(base) = crate::arch::aarch64::virtio_mmio::find_device(VIRTIO_GPU_MMIO_DEVICE_ID) {
        return probe_mmio_driver(base);
    }

    // Look for virtio-gpu on PCI bus.
    let dev = match crate::arch::pci::find_device(VIRTIO_VENDOR, VIRTIO_GPU_DEVICE_MODERN)
        .or_else(|| crate::arch::pci::find_device(VIRTIO_VENDOR, VIRTIO_GPU_DEVICE_LEGACY))
    {
        Some(d) => d,
    ...
    crate::arch::pci::enable_bus_master(dev.location);
...
#[cfg(target_arch = "aarch64")]
fn probe_mmio_driver(base: u64) -> ProbeResult {
    let mut state = STATE.lock();
    state.present = false;
    state.modern = false;
    state.io_base = 0;
    ...
    const CANDIDATE_MODES: &[(u32, u32)] =
        &[(1280, 800), (1024, 768), (800, 600), (640, 480)];
    for &(width, height) in CANDIDATE_MODES {
        let bytes = (width as usize)
            .saturating_mul(height as usize)
            .saturating_mul(4);
        let pages = bytes.div_ceil(4096);
        if let Some(phys) = crate::mm::frame_alloc::alloc_contiguous_run(pages) {
            state.width = width;
            state.height = height;
            state.fb_phys = phys;
            let _ = crate::mm::page_table::ensure_identity_mapped_2m(phys);
            let _ = crate::mm::page_table::ensure_identity_mapped_2m(
                phys.saturating_add(bytes.saturating_sub(1) as u64),
            );
            unsafe { core::ptr::write_bytes(phys as *mut u8, 0, bytes) };
            break;
        }
    }
    if state.fb_phys == 0 {
        serial::write_line(b"[virtio-gpu] ARM scanout backing allocation failed");
        return ProbeResult::Failed;
    }

    let queue = unsafe { core::ptr::addr_of_mut!(GPU_QUEUE_MEM.0) as u64 };
    let configured = unsafe {
        crate::arch::aarch64::virtio_mmio::begin_modern_device(base, 0)
            && crate::arch::aarch64::virtio_mmio::configure_queue(
                base,
                0,
                GPU_QUEUE_SIZE as u32,
                queue,
                queue + GPU_DESC_BYTES as u64,
                queue + GPU_USED_OFF as u64,
            )
            .is_some()
            && crate::arch::aarch64::virtio_mmio::finish_device(base)
    };
    if !configured || !init_scanout_locked(&mut state) {
        serial::write_line(b"[virtio-gpu] ARM MMIO scanout init failed");
        return ProbeResult::Failed;
    }
    state.present = true;
    serial::write_bytes(b"[virtio-gpu] MMIO scanout bound ");
    serial::write_u64_dec_inline(state.width as u64);
    serial::write_bytes(b"x");
    serial::write_u64_dec(state.height as u64);
    ProbeResult::Bound
}
...
fn find_legacy_io_base(bus: u8, slot: u8, func: u8) -> u16 {
    for off in [0x10u8, 0x14, 0x18, 0x1C, 0x20, 0x24] {
        let bar = crate::arch::pci::read_u32(bus, slot, func, off);
        if (bar & 0x1) == 0 {
            continue;
        }
...
fn probe_modern_cfg(bus: u8, slot: u8, func: u8) -> Option<(usize, usize, u32)> {
    if crate::arch::pci::read_u16(bus, slot, func, PCI_STATUS_OFFSET)
    ...
    let mut cap_ptr = crate::arch::pci::read_u8(bus, slot, func, PCI_CAP_PTR_OFFSET) & !0x3;
    let mut hops = 0u8;
    while cap_ptr >= 0x40 && cap_ptr != 0 && hops < 32 {
        let cap_id = crate::arch::pci::read_u8(bus, slot, func, cap_ptr);
        let next = crate::arch::pci::read_u8(bus, slot, func, cap_ptr + 1) & !0x3;
        if cap_id == PCI_CAP_ID_VENDOR_SPECIFIC {
            let cfg_type = crate::arch::pci::read_u8(bus, slot, func, cap_ptr + 3);
            let bar = crate::arch::pci::read_u8(bus, slot, func, cap_ptr + 4);
            let offset = crate::arch::pci::read_u32(bus, slot, func, cap_ptr + 8) as usize;
                ...
                match cfg_type {
                    ...
                    VIRTIO_PCI_CAP_NOTIFY_CFG => {
                        ...
                        notify_off_multiplier =
                            crate::arch::pci::read_u32(bus, slot, func, cap_ptr + 16);
                    }
                    _ => {}
                }
            }
...
fn pci_read_bar_address(bus: u8, slot: u8, func: u8, bar: u8) -> Option<u64> {
    ...
    let low = crate::arch::pci::read_u32(bus, slot, func, offset);
    if low == 0 {
        return None;
    }
    if low & 0x1 != 0 {
    ...
    let addr = match (low >> 1) & 0x3 {
        0x0 => (low & !0xF) as u64,
        0x2 if bar + 1 < 6 => {
            let high = crate::arch::pci::read_u32(bus, slot, func, offset + 4);
...
fn init_scanout_locked(state: &mut GpuState) -> bool {
    let transport_ready = if state.mmio_base != 0 {
        true
...
fn submit_controlq_locked(state: &mut GpuState, cmd: &[u8], cmd_len: usize) -> bool {
    ...
    if state.mmio_base == 0 && !state.modern && state.io_base == 0 {
        return false;
    }
    if state.mmio_base == 0 && state.modern && state.notify_addr == 0 {
        return false;
    }

    let io_base = state.io_base;
    let notify_addr = state.notify_addr;
    let mmio_base = state.mmio_base;
    ...
    crate::mm::page_table::with_kernel_address_space(|| unsafe {
        ...
        if mmio_base != 0 {
            mmio_write_u32(mmio_base as usize + 0x050, 0);
        } else if state.modern {
        ...
        let mut spins = 0u32;
        #[cfg(target_arch = "aarch64")]
        let deadline = crate::arch::aarch64::timer::read_counter().saturating_add(
            crate::arch::aarch64::timer::frequency().saturating_div(4).max(1),
        );
        loop {
            let used_idx = core::ptr::read_volatile(used_base.add(2) as *const u16);
            ...
            #[cfg(target_arch = "aarch64")]
            let timed_out = crate::arch::aarch64::timer::read_counter() >= deadline;
            #[cfg(not(target_arch = "aarch64"))]
            let timed_out = spins > 2_000_000;
            if timed_out {
...
/// submission for the LaplacianOS compositor.
///
/// LaplacianOS now accepts compositor command packets through `SYS_GPU_SUBMIT`
...
pub fn blit_surface_scene(
    ...
    let dst_w = ((src_w as u64 * scale as u64) / 1024) as i32;
    let dst_h = ((src_h as u64 * scale as u64) / 1024) as i32;
    if dst_w <= 0 || dst_h <= 0 {
        return;
    }
...
const MAX_HOST_RESOURCES: usize = 256;
...
impl HostResource {
    const EMPTY: Self = Self {
        ...
        height: 0,
        format: 0,
        backing_first_phys: 0,
        backing_bytes: 0,
    };
}

static HOST_RESOURCES: Mutex<[HostResource; MAX_HOST_RESOURCES]> =
    Mutex::new([HostResource::EMPTY; MAX_HOST_RESOURCES]);

fn virtio_2d_format(format: u8) -> Option<u32> {
    match format {
        0 => Some(1),  // B8G8R8A8_UNORM
        1 => Some(67), // R8G8B8A8_UNORM
        _ => None,
    }
}
...
///
pub fn resource_create_2d(width: u32, height: u32, format: u8) -> Option<u32> {
    if width == 0 || height == 0 || width > 8192 || height > 8192 {
        return None;
    }
    let host_format = virtio_2d_format(format)?;
    let mut resources = HOST_RESOURCES.lock();
    let slot = resources.iter().position(|resource| resource.id == 0)?;
    ...
    let mut cmd = [0u8; GPU_CMD_BUF_SIZE];
    cmd[0..4].copy_from_slice(&VIRTIO_GPU_CMD_RESOURCE_CREATE_2D.to_le_bytes());
    cmd[24..28].copy_from_slice(&id.to_le_bytes());
    cmd[28..32].copy_from_slice(&host_format.to_le_bytes());
    cmd[32..36].copy_from_slice(&width.to_le_bytes());
    cmd[36..40].copy_from_slice(&height.to_le_bytes());
    if !submit_controlq_locked(&mut state, &cmd, 40) {
        return None;
    }
    resources[slot] = HostResource {
        id,
        width,
        height,
        format,
...
///
pub fn resource_destroy(resource_id: u32) -> bool {
    if resource_id < 2 {
        return false;
    }
    let mut resources = HOST_RESOURCES.lock();
    let Some(slot) = resources.iter().position(|resource| resource.id == resource_id) else {
        return false;
    };
    ...
    if resource.backing_bytes != 0 {
        let mut detach = [0u8; GPU_CMD_BUF_SIZE];
        detach[0..4].copy_from_slice(&VIRTIO_GPU_CMD_RESOURCE_DETACH_BACKING.to_le_bytes());
        detach[24..28].copy_from_slice(&resource_id.to_le_bytes());
        if !submit_controlq_locked(&mut state, &detach, 28) {
            return false;
        }
    }
    let mut unref = [0u8; GPU_CMD_BUF_SIZE];
    unref[0..4].copy_from_slice(&VIRTIO_GPU_CMD_RESOURCE_UNREF.to_le_bytes());
    unref[24..28].copy_from_slice(&resource_id.to_le_bytes());
    if submit_controlq_locked(&mut state, &unref, 28) {
        resources[slot] = HostResource::EMPTY;
        true
    } else {
        false
    }
...
/// Attach physical backing pages to a GPU resource so it can be accessed as a
/// texture without a CPU copy.
///
/// Consecutive pages are coalesced into virtio memory entries. The resource is
/// attached only when the supplied pages cover its complete pixel payload.
pub fn resource_attach_backing_pages(resource_id: u32, pages: &[u64]) -> bool {
    if resource_id < 2 || pages.is_empty() {
        return false;
    }
    if pages
        .iter()
        .any(|phys| *phys == 0 || *phys & 0xfff != 0)
    {
        return false;
    }
    let mut resources = HOST_RESOURCES.lock();
    ...
    if resources[slot].backing_bytes != 0 {
        return false;
    }
    let resource = resources[slot];
    ...
    let required_bytes = (resource.width as u64)
        .checked_mul(resource.height as u64)
        .and_then(|pixels| pixels.checked_mul(bytes_per_pixel));
    let Some(required_bytes) = required_bytes else {
        return false;
    };
    let supplied_bytes = (pages.len() as u64).saturating_mul(4096);
    if supplied_bytes < required_bytes {
        return false;
    }

    const ENTRY_BYTES: usize = 16;
    const ATTACH_HEADER_BYTES: usize = 32;
    const MAX_ENTRIES: usize = (GPU_CMD_BUF_SIZE - ATTACH_HEADER_BYTES) / ENTRY_BYTES;
    let mut runs = [(0u64, 0u32); MAX_ENTRIES];
    let mut run_count = 0usize;
    for &phys in pages {
        if run_count != 0 {
            let (run_start, run_len) = runs[run_count - 1];
            if run_start.saturating_add(run_len as u64) == phys
                && run_len <= u32::MAX - 4096
            {
                runs[run_count - 1].1 = run_len + 4096;
                continue;
            }
        }
    ...
    let mut attach = [0u8; GPU_CMD_BUF_SIZE];
    attach[0..4].copy_from_slice(&VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING.to_le_bytes());
    attach[24..28].copy_from_slice(&resource_id.to_le_bytes());
    attach[28..32].copy_from_slice(&(run_count as u32).to_le_bytes());
    for (index, &(phys, length)) in runs[..run_count].iter().enumerate() {
        let offset = ATTACH_HEADER_BYTES + index * ENTRY_BYTES;
        attach[offset..offset + 8].copy_from_slice(&phys.to_le_bytes());
        attach[offset + 8..offset + 12].copy_from_slice(&length.to_le_bytes());
    }
    let command_bytes = ATTACH_HEADER_BYTES + run_count * ENTRY_BYTES;
    if !submit_controlq_locked(&mut state, &attach, command_bytes) {
        return false;
    }
    resources[slot].backing_first_phys = pages[0];
    resources[slot].backing_bytes = supplied_bytes;
    true
}

/// Attach one physical page. This is useful for small cursor and control
/// textures; full surfaces should use `resource_attach_backing_pages`.
pub fn resource_attach_backing(resource_id: u32, phys_addr: u64) -> bool {
    resource_attach_backing_pages(resource_id, core::slice::from_ref(&phys_addr))
}
