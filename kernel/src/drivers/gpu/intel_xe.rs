...
fn read_bar0_64(bus: u8, slot: u8, func: u8) -> Option<u64> {
    let low = crate::arch::pci::read_u32(bus, slot, func, PCI_BAR0_OFFSET);
    ...
    match bar_type {
        0x0 => Some(base_low),
        0x2 => {
            let high = crate::arch::pci::read_u32(bus, slot, func, PCI_BAR0_OFFSET + 4);
...
#[inline]
fn is_intel_display(info: &crate::arch::pci::PciDeviceInfo) -> bool {
...
pub fn probe_driver() -> ProbeResult {
    let mut candidate: Option<crate::arch::pci::PciDeviceInfo> = None;
    ...
    let mut cmd = crate::arch::pci::read_u16(
        dev.location.bus,
        dev.location.slot,
        dev.location.func,
