...
pub fn poll() {
    // QEMU's AArch64 virt machine exposes no smart-battery or EC node. Keep
    // the record explicitly absent; never synthesize charge telemetry.
    *BATTERY.lock() = BatteryStatus::absent();
}
