...
fn test_ramfs_round_trip() -> bool {
    ...
    let payload = b"LaplacianOS MVP path is writable";
...
fn test_posixfs_graph_alias_round_trip() -> bool {
    let path = b"/fs/var/lib/laplacianos/compat.txt";
    ...
    let fd = match vfs::create(path) {
        Ok(fd) => fd,
        Err(_) => {
            diag::test_fail(b"vfs: create /fs/var/lib/laplacianos/compat.txt");
            return false;
        }
    };
    ...
    match vfs::write(fd, payload) {
        ...
        _ => {
            let _ = vfs::close(fd);
            diag::test_fail(b"vfs: write /fs/var/lib/laplacianos/compat.txt");
            return false;
        }
    }
    let _ = vfs::close(fd);
    ...
    let fd = match vfs::open(path) {
        Ok(fd) => fd,
        Err(_) => {
            diag::test_fail(b"vfs: reopen /fs/var/lib/laplacianos/compat.txt");
            return false;
        }
    };
