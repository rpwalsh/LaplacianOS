...
fn kick_frame_tick_channel(channel_alias: u32) {
    ...
    if matches!(
        send_frame_tick(channel_alias, &payload),
        FrameTickDispatch::Delivered
    ) {
...
pub fn spawn_task(test_failures: u32, protected_ok: bool) -> bool {
    ...
    // The protected compositor can claim the display and subscribe before the
    // kernel display-broker task is spawned. Preserve that live handoff state;
    // clearing it here loses the only subscriber and leaves the desktop
    // waiting forever after an already-successful first present.
...
impl Desktop {
    fn new(_config: FramebufferConfig, _test_failures: u32, _protected_ok: bool) -> Self {
        ...
        if let Some(binding) = registry::lookup(b"compositor")
            && compositor_registry_live(&binding)
        {
            crate::arch::serial::write_line(b"[desktop] compositor already registered at boot\n");
        }
    ...
    fn autostart_ai_console(&mut self) {
        if self.ai_console_autostarted {
            return;
        }
        self.ai_console_autostarted = true;

        if userland::spawn_named_service(b"review-ai").is_some() {
            crate::arch::serial::write_line(b"[desktop] review AI surface autostarted\n");
        } else {
            crate::arch::serial::write_line(
                b"[desktop] WARNING: review AI surface autostart failed\n",
            );
        }
    }
