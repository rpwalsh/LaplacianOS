...
//!
//! ## Boot task (index 0)
//! The boot thread (laplacianos_kmain) is not a "real" task — it is the code
...
unsafe fn install_runtime_state_for(index: usize) {
    ...
    if stack_top != 0 {
        ...
        crate::arch::user::set_syscall_kernel_stack(stack_top);
    }
...
pub unsafe fn preempt() {
    ...
    if !INITIALIZED.load(Ordering::Acquire) {
        finish_preempt_interrupt_state();
        return;
    }
    ...
    // Twin-informed bounded adaptive slice sizing.
    let _hint = crate::graph::twin::query_sched_hint();

    // Feed the telemetry facade at each context switch.
...
#[inline]
fn finish_preempt_interrupt_state() {
    #[cfg(target_arch = "x86_64")]
    crate::arch::cpu::enable_interrupts();
    #[cfg(target_arch = "aarch64")]
    {
        // The AArch64 caller is the IRQ vector. RESTORE_GPRS reloads the
        // suspended task's SPSR_EL1 and ERET restores its interrupt mask
        // atomically; unmasking here would permit nested IRQs on a partial
        // exception frame.
    }
}
...
unsafe fn terminate_current_task(log_reason: &[u8]) -> ! {
    ...
    match table::find_next_ready(cur) {
        Some(idx) => {
            ...
            #[cfg(target_arch = "aarch64")]
            unsafe {
                serial::write_bytes(b"[sched] ARM switch target idx=");
                serial::write_u64_dec_inline(idx as u64);
                serial::write_bytes(b" pc=");
                serial::write_hex_inline((*new_ctx).pc);
                serial::write_bytes(b" sp=");
                serial::write_hex_inline((*new_ctx).sp);
                serial::write_bytes(b" lr=");
                serial::write_hex((*new_ctx).x30);
            }
...
fn user_task_bootstrap() {
    ...
    #[cfg(target_arch = "aarch64")]
    unsafe {
        let first_instruction = core::ptr::read_volatile(user_rip as *const u32);
        serial::write_bytes(b"[sched] ARM EL0 first instruction=");
        serial::write_hex(first_instruction as u64);
    }

    let thread_arg = table::task_thread_arg_at(cur);
    if thread_arg != 0 {
        unsafe {
            crate::arch::user::enter_user_mode_with_arg(user_rip, user_rsp, thread_arg)
        }
    } else {
        unsafe { crate::arch::user::enter_user_mode(user_rip, user_rsp) }
    }
}
