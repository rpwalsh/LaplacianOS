...
static HARDWARE_IDS: Mutex<[u32; MAX_CPUS]> = Mutex::new([u32::MAX; MAX_CPUS]);
...
pub fn register_cpu(hardware_id: u32) -> usize {
    ...
    let mut mapping = HARDWARE_IDS.lock();
    // Check if already registered (idempotent for BSP double-calls).
    if let Some(index) = mapping.iter().position(|&id| id == hardware_id) {
        return index;
    }
    // Find next free slot.
    let idx = {
...
pub fn current_cpu_index() -> usize {
    let hardware_id = crate::arch::cpu::hardware_thread_id();
    HARDWARE_IDS
        .lock()
        .iter()
...
pub fn init_percpu(cpu_count: usize) {
    ...
    let bsp_lapic = crate::arch::cpu::hardware_thread_id();
    {
        let mut mapping = HARDWARE_IDS.lock();
        mapping[0] = bsp_lapic;
    }
