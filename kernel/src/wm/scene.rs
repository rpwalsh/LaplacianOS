//! Retained-mode scene graph for the LaplacianOS GPU compositor.
//!
//! ## Architecture
//!
...
//!        └─ NativeGpuBackend (planned — LaplacianOS-native textured quads)
//! ```
//!
//! ## Design constraints
...
impl SceneGraph {
    ...
    fn rebuild_sort(&mut self) {
        ...
        for (target, &(_, node_index)) in self.sorted.iter_mut().zip(pairs.iter()).take(n) {
            *target = node_index;
        }
        self.sorted_len = n;
...
impl AnimationTimeline {
    ...
    fn slot_for(&self, id: NodeId) -> Option<usize> {
        (0..MAX_ANIMATIONS).find(|&index| {
            self.anims[index]
                .as_ref()
                .map(|animation| animation.node_id)
                == Some(id)
        })
    }
}
