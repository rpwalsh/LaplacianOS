...
/// Return an owned snapshot of all physical frames backing a surface.
///
/// This avoids placing the maximum 32 KiB frame array on a kernel stack in
/// consumers that need to submit the complete backing list to a device.
pub fn surface_frame_list(id: u32) -> Option<Vec<u64>> {
    interrupts::without_interrupts(|| {
        let tbl = TABLE.lock();
        tbl.records
            .iter()
            .find(|record| record.active && record.id == id && record.frame_count != 0)
            .map(|record| record.frames[..record.frame_count].to_vec())
    })
}
