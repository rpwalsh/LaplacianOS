...
fn ln_integer_q16(value: u64) -> u64 {
    ln_q16(value.saturating_mul(WEIGHT_ONE as u64))
}
...
impl ReconstructionError {
    ...
    pub fn update_baseline(&mut self, error: Weight) {
        self.baseline_count = self.baseline_count.saturating_add(1);
        let n = self.baseline_count;

        if n == 1 {
            self.baseline_mean = error;
            self.baseline_stddev = 0;
            self.baseline_m2_q32 = 0;
            return;
        }

        // Welford's: delta = x - mean
        let old_mean = self.baseline_mean as i128;
        let delta = error as i128 - old_mean;

        // new_mean = mean + delta/n
        let new_mean = old_mean + delta / n as i128;
        self.baseline_mean = new_mean.clamp(0, Weight::MAX as i128) as Weight;
        ...
        let delta_two = error as i128 - new_mean;
        self.baseline_m2_q32 = self
            .baseline_m2_q32
            .saturating_add(delta.saturating_mul(delta_two));
        let variance_q32 = (self.baseline_m2_q32.max(0) as u128) / (n - 1) as u128;
        self.baseline_stddev = integer_sqrt_u128(variance_q32).min(Weight::MAX as u128) as Weight;
    }
}
...
impl CooccurrenceSlice {
    ...
    pub fn observe(&mut self, i: usize, j: usize) {
        if i < TENSOR_MAX_DIM && j < TENSOR_MAX_DIM {
            self.counts[i][j] = self.counts[i][j].saturating_add(1);
            self.row_sums[i] = self.row_sums[i].saturating_add(1);
            self.col_sums[j] = self.col_sums[j].saturating_add(1);
    ...
    pub fn ppmi(&self, i: usize, j: usize) -> Weight {
        ...
        let pmi_ln_q16 = ln_integer_q16(c_ij) as i128 + ln_integer_q16(self.total) as i128
            - ln_integer_q16(r_i) as i128
            - ln_integer_q16(c_j) as i128;
        if pmi_ln_q16 <= 0 {
            return 0;
        }
        ((((pmi_ln_q16 as u128) << 16) / LN_2_Q16 as u128).min(Weight::MAX as u128)) as Weight
