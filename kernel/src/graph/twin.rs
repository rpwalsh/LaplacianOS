...
//! typed edges carrying real-time telemetry. The existing LaplacianOS
...
//!
//! ## Observed-state and predictive model
//!
//! - **Observed-state histograms**: Each twin node maintains a histogram of
//!   measured states for diagnostics. Forecasts and prediction intervals are
//!   stored separately and are never inferred from the historical histogram.
...
//! - `causal::CausalGraph` discoveries update validated lagged dependencies.
...
//!   and automatically degrades validation support when the model is wrong.
...
/// Histogram bins for observed-state diagnostics.
...
/// Histogram over discretized observed sensor values.
///
/// Each bin covers an equal range of the sensor's [min, max] domain.
/// Counts represent unnormalized historical frequency. The `collapse`
/// operation returns the observed weighted mean; it is not a forecast.
///
/// 80 bytes, repr(C), Copy.
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct ObservedStateHistogram {
...
/// observed-state histogram, forecast intervals, and error tracking.
pub struct TwinSensor {
    ...
    ema: Weight,
    /// Histogram of observed values. This is descriptive, not predictive.
    distribution: ObservedStateHistogram,
    /// Predicted values for the next PREDICTION_HORIZON steps.
    predictions: [Weight; PREDICTION_HORIZON],
    /// Half-width of the 95% prediction interval for each horizon.
    prediction_intervals: [Weight; PREDICTION_HORIZON],
    /// Validation support derived from sample count and recent error.
    /// This is not a calibrated probability of correctness.
    prediction_support: Weight,
...
impl TwinSensor {
    pub const fn empty() -> Self {
        Self {
            ...
            count: 0,
            ema: 0,
            distribution: ObservedStateHistogram::empty(0, WEIGHT_ONE),
            predictions: [0; PREDICTION_HORIZON],
            prediction_intervals: [0; PREDICTION_HORIZON],
    ...
    /// Returns validation support (0–WEIGHT_ONE), not a correctness probability.
    pub fn support(&self) -> Weight {
        self.prediction_support
    }
    /// Returns the 95% prediction-interval half-width for each horizon.
    pub fn prediction_intervals(&self) -> &[Weight; PREDICTION_HORIZON] {
    ...
    fn reset(&mut self) {
        ...
        self.prediction_intervals = [0; PREDICTION_HORIZON];
        self.prediction_support = 0;
        ...
        self.distribution = ObservedStateHistogram::empty(
    ...
    pub fn observe(&mut self, timestamp: Timestamp, value: Weight) {
        ...
        // Update the descriptive observed-state histogram.
    ...
    /// Timestamped least-squares forecast with horizon-specific intervals.
    ///
    /// Timestamps are centered before multiplication to keep the fixed-size
    /// integer implementation bounded. Validation support is derived from
    /// sample count and observed one-step error; it is not called confidence.
    fn predict_linear(&mut self) {
        let n = self.count.min(32);
        if n < 2 {
            return;
        }
        let first = (self.head + SENSOR_HISTORY - n) % SENSOR_HISTORY;
        let first_timestamp = self.history[first].timestamp;
        let mut times = [0i128; 32];
        let mut values = [0i128; 32];
        let mut sum_t = 0i128;
        let mut sum_y = 0i128;
        for index in 0..n {
            let observation = self.history[(first + index) % SENSOR_HISTORY];
            times[index] = observation.timestamp.saturating_sub(first_timestamp) as i128;
            values[index] = observation.smoothed as i128;
            sum_t += times[index];
            sum_y += values[index];
        }

        let n_i128 = n as i128;
        let mean_t = sum_t / n_i128;
        let mean_y = sum_y / n_i128;
        let mut covariance = 0i128;
        let mut time_variance = 0i128;
        for index in 0..n {
            let centered_t = times[index] - mean_t;
            covariance =
                covariance.saturating_add(centered_t.saturating_mul(values[index] - mean_y));
            time_variance = time_variance.saturating_add(centered_t.saturating_mul(centered_t));
        }

        let latest_timestamp =
            self.history[(self.head + SENSOR_HISTORY - 1) % SENSOR_HISTORY].timestamp;
        let elapsed = latest_timestamp.saturating_sub(first_timestamp);
        let step = (elapsed / (n as u64 - 1)).max(1);
        let predict_at = |relative_timestamp: i128| -> i128 {
            if time_variance == 0 {
                mean_y
            } else {
                mean_y + covariance.saturating_mul(relative_timestamp - mean_t) / time_variance
            }
        };

        let mut residual_sum_squares = 0u128;
        if n > 2 && time_variance != 0 {
            for index in 0..n {
                let residual = values[index] - predict_at(times[index]);
                residual_sum_squares =
                    residual_sum_squares.saturating_add(residual.saturating_mul(residual) as u128);
            }
        }
        let residual_variance = if n > 2 {
            residual_sum_squares / (n - 2) as u128
        } else {
            0
        };

        for horizon in 0..PREDICTION_HORIZON {
            let relative_timestamp =
                elapsed.saturating_add(step.saturating_mul(horizon as u64 + 1)) as i128;
            self.predictions[horizon] =
                predict_at(relative_timestamp).clamp(0, Weight::MAX as i128) as Weight;

            if residual_variance == 0 || time_variance == 0 {
                self.prediction_intervals[horizon] = 0;
                continue;
            }
            let centered_t = relative_timestamp - mean_t;
            let leverage_q16 = WEIGHT_ONE as u128
                + WEIGHT_ONE as u128 / n as u128
                + ((centered_t.saturating_mul(centered_t) as u128) << 16) / time_variance as u128;
            let scaled_variance = residual_variance.saturating_mul(leverage_q16) >> 16;
            // Approximate 1.96 standard errors with the exact rational 49/25.
            self.prediction_intervals[horizon] = integer_sqrt_u128(scaled_variance)
                .saturating_mul(49)
                .saturating_div(25)
                .min(Weight::MAX as u128)
                as Weight;
        }

        let sample_support = ((n.min(32) as u64 * WEIGHT_ONE as u64) / 32) as Weight;
        let domain_range = self
            .distribution
            .domain_max()
            .saturating_sub(self.distribution.domain_min())
            .max(1) as u64;
        let relative_error = ((self.prediction_error as u64 * WEIGHT_ONE as u64) / domain_range)
            .min(WEIGHT_ONE as u64) as Weight;
        self.prediction_support = ((sample_support as u64
            * WEIGHT_ONE.saturating_sub(relative_error) as u64)
            >> 16) as Weight;
    }
...
// ────────────────────────────────────────────────────────────────────
// Validated lagged dependency between twin sensors
// ────────────────────────────────────────────────────────────────────

/// A causal dependency between two twin sensors.
///
/// The link is activated only after an out-of-sample predictive dependency
/// test. `direction` preserves the sign of the response and `lag` identifies
/// the forecast horizon to which the adjustment applies.
///
...
///
/// Holds all sensor nodes, validated lagged links, prediction state, and
...
impl TwinState {
    ...
    pub fn register_sensor(
        ...
        self.sensors[idx].distribution = ObservedStateHistogram::empty(domain_min, domain_max);
        self.sensor_count += 1;
        Some(idx)
    }
    ...
    pub fn observe(&mut self, sensor_idx: usize, timestamp: Timestamp, value: Weight) {
        ...
        let previous_source = self.sensors[sensor_idx].ema();
        self.sensors[sensor_idx].observe(timestamp, value);

        // Propagate through validated lagged links.
        self.propagate_causal(sensor_idx, previous_source, value);
    ...
    /// receives a signed adjustment at the link's validated lag only.
    fn propagate_causal(&mut self, source_idx: usize, previous_source: Weight, new_value: Weight) {
        for i in 0..self.link_count {
            ...
            let horizon = link.lag() as usize;
            if horizon >= PREDICTION_HORIZON || link.direction() == 0 {
                continue;
            }
            let delta = new_value as i128 - previous_source as i128;
            let signed_adjustment = delta
                .saturating_mul(link.coupling() as i128)
                .saturating_div(WEIGHT_ONE as i128)
                .saturating_mul(link.direction() as i128);
            let prediction = self.sensors[dest_idx].predictions[horizon] as i128;
            self.sensors[dest_idx].predictions[horizon] = prediction
                .saturating_add(signed_adjustment)
                .clamp(0, Weight::MAX as i128)
                as Weight;
            self.sensors[dest_idx].prediction_intervals[horizon] =
                self.sensors[dest_idx].prediction_intervals[horizon].saturating_add(
                    signed_adjustment.unsigned_abs().min(Weight::MAX as u128) as Weight,
                );
        }
    }
    ...
    /// validated dependency links backwards.
    ///
    ...
    ///
    /// Returns `(predicted_value, validation_support)` for the horizon.
    pub fn query_prediction(&self, sensor_idx: usize, horizon: usize) -> (Weight, Weight) {
        ...
        (sensor.predictions()[horizon], sensor.support())
    }

    /// Query the mean of the observed-state histogram.
    ...
    pub fn dump(&self) {
        ...
        for i in 0..self.sensor_count {
            ...
            serial::write_bytes(b" support=");
            serial::write_u64_dec_inline(s.support() as u64);
...
pub struct SchedHint {
    ...
    /// Whether validation support is sufficient to apply the bounded hint.
    pub supported: bool,
}
...
/// - **Coherence boost**: if system state is stable and validation support is
///   sufficient, allow a bounded extended quantum.
...
pub fn query_sched_hint() -> SchedHint {
    ...
    // Require enough observations to support a scheduling recommendation.
    let temp_idx = SENSOR_CPU_TEMP.load(Ordering::Acquire);
