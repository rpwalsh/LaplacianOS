...
pub fn init(display_width: u32, display_height: u32) -> bool {
    ...
    crate::arch::input::set_display_bounds(display_width, display_height);
    let virtio_arch_ready = crate::arch::input::init();
    let virtio_driver_ready = legacy_virtio_input_present();
    let ps2_ready = ps2_init();
...
pub fn set_display_bounds(width: u32, height: u32) {
    ...
    crate::arch::input::set_display_bounds(width, height);
}
...
pub fn irq_line() -> Option<u8> {
    ...
    if state.virtio_arch_ready {
        return crate::arch::input::irq_line();
    }
    if state.virtio_driver_ready {
        return legacy_virtio_input_irq();
    }
    if state.ps2_ready {
        return Some(12);
    }
...
pub fn handle_irq(irq: u8) -> bool {
    ...
    if state.virtio_arch_ready {
        handled |= crate::arch::input::handle_irq(irq);
    }
    ...
    if state.virtio_driver_ready && legacy_virtio_input_irq() == Some(irq) {
        #[cfg(target_arch = "x86_64")]
...
fn backend_has_pending(backend: PointerBackend) -> bool {
    match backend {
        PointerBackend::VirtioArch => crate::arch::input::has_pending_event(),
        PointerBackend::VirtioDriver => legacy_virtio_input_pending(),
        PointerBackend::Ps2 => ps2_has_pending_event(),
        PointerBackend::None => false,
    }
}
...
fn try_backend_event(backend: PointerBackend) -> Option<PointerEvent> {
    match backend {
        PointerBackend::VirtioArch => crate::arch::input::try_read_event(),
...
#[cfg(target_arch = "x86_64")]
fn ps2_init() -> bool { crate::arch::x86_64::mouse::init() }
#[cfg(target_arch = "aarch64")]
fn ps2_init() -> bool { false }
#[cfg(target_arch = "x86_64")]
fn ps2_handle_interrupt() { crate::arch::x86_64::mouse::handle_interrupt() }
#[cfg(target_arch = "x86_64")]
fn ps2_poll_input() { crate::arch::x86_64::mouse::poll_input() }
#[cfg(target_arch = "x86_64")]
fn ps2_has_pending_event() -> bool { crate::arch::x86_64::mouse::has_pending_event() }
#[cfg(target_arch = "aarch64")]
fn ps2_has_pending_event() -> bool { false }
...
#[cfg(target_arch = "x86_64")]
fn legacy_virtio_input_present() -> bool { crate::drivers::input::virtio_input::is_present() }
#[cfg(target_arch = "aarch64")]
fn legacy_virtio_input_present() -> bool { false }
#[cfg(target_arch = "x86_64")]
fn legacy_virtio_input_irq() -> Option<u8> { crate::drivers::input::virtio_input::irq_line() }
#[cfg(target_arch = "aarch64")]
fn legacy_virtio_input_irq() -> Option<u8> { None }
#[cfg(target_arch = "x86_64")]
fn legacy_virtio_input_pending() -> bool { crate::drivers::input::virtio_input::has_pending_event() }
#[cfg(target_arch = "aarch64")]
fn legacy_virtio_input_pending() -> bool { false }
