...
pub fn create_kernel_task(name: &[u8], entry_fn: u64) -> Option<TaskId> {
    ...
    let adjusted_stack_top = stack_top;
    #[cfg(target_arch = "x86_64")]
    unsafe {
    ...
    let ctx = CpuContext::new_kernel(
        entry_fn,
        adjusted_stack_top,
        trampoline_addr,
        super::switch::kernel_task_start_addr(entry_fn),
    );
...
fn finalize_user_task(
    ...
    let ctx = CpuContext::new_kernel(
        crate::sched::user_task_bootstrap_addr(),
        adjusted_stack_top,
        trampoline_addr,
        crate::sched::user_task_bootstrap_addr(),
    );
...
pub fn handle_user_page_fault(index: usize, fault_addr: u64, error_code_bits: u64) -> bool {
    ...
    if let Some(space) = task.address_space.as_mut() {
    ...
    for slot in table.tasks.iter_mut() {
        ...
        if t.cr3 == cr3
            && let Some(space) = t.address_space.as_mut()
        {
            return space.handle_page_fault(fault_addr, error_code_bits);
        }
    }
