...
use object::elf::{FileHeader64, PF_W, PF_X, PT_LOAD, SHN_UNDEF, SHT_RELA};
#[cfg(target_arch = "x86_64")]
use object::elf::{
    R_X86_64_32 as R_ELF_32, R_X86_64_32S as R_ELF_32S, R_X86_64_64 as R_ELF_64,
    R_X86_64_GLOB_DAT as R_ELF_GLOB_DAT, R_X86_64_JUMP_SLOT as R_ELF_JUMP_SLOT,
    R_X86_64_NONE as R_ELF_NONE, R_X86_64_RELATIVE as R_ELF_RELATIVE,
    R_X86_64_RELATIVE64 as R_ELF_RELATIVE_ALT,
};
#[cfg(target_arch = "aarch64")]
use object::elf::{
    R_AARCH64_ABS32 as R_ELF_32, R_AARCH64_ABS64 as R_ELF_64,
    R_AARCH64_GLOB_DAT as R_ELF_GLOB_DAT, R_AARCH64_JUMP_SLOT as R_ELF_JUMP_SLOT,
    R_AARCH64_NONE as R_ELF_NONE, R_AARCH64_RELATIVE as R_ELF_RELATIVE,
};
#[cfg(target_arch = "aarch64")]
const R_ELF_32S: u32 = u32::MAX - 1;
#[cfg(target_arch = "aarch64")]
const R_ELF_RELATIVE_ALT: u32 = u32::MAX;
use object::read::elf::{
...
impl AddressSpace {
    ...
    pub fn new(kernel_pml4_phys: u64) -> Option<Box<Self>> {
        ...
        let aslr_pages = crate::arch::entropy::random_u64()
    ...
    pub fn load_elf_image(&mut self, image: &[u8]) -> Option<UserImageInfo> {
        ...
        let aslr_offset = crate::arch::entropy::random_u64()
        ...
        for phdr in phdrs {
            ...
            if seg_end > eager_end
                && !self.register_region(eager_end, seg_end, page_flags, VmRegionKind::Load, true)
            {
    ...
    fn map_page_unowned(&mut self, vaddr: u64, phys: u64, page_flags: u64) -> bool {
        use crate::arch::paging::{pd_index, pdpt_index, pml4_index, pt_index};
        ...
        unsafe {
            ...
            if entry_ptr.read() & crate::arch::paging::flags::PRESENT != 0 {
                return false; // already mapped
            }
            entry_ptr.write(crate::arch::paging::encode_leaf(phys, page_flags));
        }
        true
    }
    ...
    pub fn map_page(&mut self, vaddr: u64, phys: u64, page_flags: u64) -> bool {
        ...
        unsafe {
            ...
            let pte_value = crate::arch::paging::encode_leaf(phys, page_flags);
    ...
    fn resolve_relocation_value(&self, query: ElfRelocationQuery<'_>) -> Option<u64> {
        ...
        match query.r_type {
            R_ELF_RELATIVE | R_ELF_RELATIVE_ALT => addend_u64?.checked_add(query.load_bias),
            R_ELF_64 | R_ELF_32 | R_ELF_32S | R_ELF_GLOB_DAT | R_ELF_JUMP_SLOT => {
    ...
    fn find_free_range(&mut self, len: u64) -> Option<u64> {
        ...
        let entropy_pages = {
            let rand = crate::arch::entropy::random_u64().unwrap_or_else(|| {
    ...
    fn break_cow_page(&mut self, page: u64, page_flags: u64) -> bool {
        ...
        unsafe {
            ...
            entry_ptr.write(crate::arch::paging::encode_leaf(new_frame, page_flags));
    ...
    unsafe fn ensure_table(&mut self, table: *mut u64, index: usize) -> u64 {
        ...
        unsafe {
            table.add(index).write(crate::arch::paging::encode_table(
                frame,
