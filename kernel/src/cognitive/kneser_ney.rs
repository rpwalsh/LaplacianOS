...
impl TokenEntry {
    ...
    fn matches(&self, hash: u64, token: &[u8]) -> bool {
        self.hash == hash
            && self.length as usize == token.len()
            && self.bytes[..token.len()] == token[..]
    }
}
...
pub struct KneserNeyModel {
    dictionary: [TokenEntry; MAX_VOCAB],
    token_count: usize,
...
impl KneserNeyModel {
    pub const fn new() -> Self {
        Self {
            dictionary: [TokenEntry::EMPTY; MAX_VOCAB],
            token_count: 0,
    ...
    pub fn observe_trigram(&mut self, w0: u16, w1: u16, w2: u16) {
        if w0 as usize >= MAX_VOCAB || w1 as usize >= MAX_VOCAB || w2 as usize >= MAX_VOCAB {
            return;
        }
        self.total_tokens += 1;
        ...
        // Update bigram (w0, w1).
        self.inc_bigram(w0, w1);
        self.inc_bigram(w1, w2);
        ...
        self.unigrams[w2 as usize].continuation_count =
            self.distinct_predecessors(w2).min(u16::MAX as u32) as u16;
        self.total_continuation = self.bigram_count.min(u32::MAX as usize) as u32;
    ...
    pub fn observe_text(&mut self, text: &[u8]) -> u32 {
        ...
        while start < len {
            ...
            if start > word_start
                && let Some(token) = self.intern_token(&text[word_start..start])
            {
                tokens.push(token);
            }
        }
    ...
    pub fn probability(&self, w0: u16, w1: u16, w2: u16) -> Weight {
        ...
        let bi_count = self.trigram_context_count(w0, w1);
        ...
        if bi_count > 0 {
            let discounted = if tri_count > 0 {
                ...
                fp_div(sub, bi_count << 16)
            } else {
                0
            };

            // Interpolation weight γ = D * N_{1+}(w0, w1, •) / c(w0, w1)
            let gamma = fp_mul(DISCOUNT, fp_div(bi_contin << 16, bi_count << 16));
    ...
    fn bigram_probability(&self, w1: u16, w2: u16) -> Weight {
        ...
        let bi_count = self.distinct_trigram_predecessors(w1, w2);
        let ctx_count = self.distinct_trigram_context_total(w1);

        if ctx_count > 0 {
            let discounted = if bi_count > 0 {
                let bc_fp = bi_count << 16;
                let sub = bc_fp.saturating_sub(DISCOUNT);
                fp_div(sub, ctx_count << 16)
            } else {
                0
            };
            let ctx_contin = self.bigram_context_continuation(w1);
            let gamma = if ctx_count > 0 {
                fp_mul(DISCOUNT, fp_div(ctx_contin << 16, ctx_count << 16))
            } else {
                0
            };
    ...
    fn unigram_probability(&self, w: u16) -> Weight {
        ...
        let cont = self.distinct_predecessors(w);
        fp_div(cont, self.bigram_count as u32)
    }

    /// Score a sequence of tokens.  Returns the sum of log-probabilities
    /// (in 16.16 fixed-point, approximated).
    pub fn score_sequence(&self, tokens: &[u16]) -> i64 {
        if tokens.len() < 3 {
            return 0;
        }
        let mut total = 0i64;
        ...
        while i + 2 < tokens.len() {
            ...
            let inverse_q16 = ((1u128 << 32) / p as u128).min(u64::MAX as u128) as u64;
            total = total.saturating_sub(ln_q16(inverse_q16) as i64);
            i += 1;
        }
        total
    }

    pub fn token_id(&self, token: &[u8]) -> Option<u16> {
        if token.is_empty() || token.len() > MAX_TOKEN_BYTES {
            return None;
        }
        let hash = fnv1a64(token);
        (0..self.token_count)
            .find(|index| self.dictionary[*index].matches(hash, token))
            .map(|index| index as u16)
    }

    fn intern_token(&mut self, token: &[u8]) -> Option<u16> {
        if let Some(id) = self.token_id(token) {
            return Some(id);
        }
        if token.is_empty() || token.len() > MAX_TOKEN_BYTES || self.token_count >= MAX_VOCAB {
            return None;
        }
        let id = self.token_count;
        let mut bytes = [0; MAX_TOKEN_BYTES];
        bytes[..token.len()].copy_from_slice(token);
        self.dictionary[id] = TokenEntry {
            hash: fnv1a64(token),
            length: token.len() as u8,
            bytes,
        };
        self.token_count += 1;
        Some(id as u16)
    ...
    fn trigram_context_count(&self, w0: u16, w1: u16) -> u32 {
        self.trigrams
            .iter()
            .filter(|entry| !entry.is_empty() && entry.w0 == w0 && entry.w1 == w1)
            .fold(0u32, |sum, entry| sum.saturating_add(entry.count as u32))
    }

    fn distinct_trigram_predecessors(&self, w1: u16, w2: u16) -> u32 {
        self.trigrams
            .iter()
            .filter(|entry| !entry.is_empty() && entry.w1 == w1 && entry.w2 == w2)
            .count()
            .min(u32::MAX as usize) as u32
    }

    fn distinct_trigram_context_total(&self, w1: u16) -> u32 {
        self.trigrams
            .iter()
            .filter(|entry| !entry.is_empty() && entry.w1 == w1)
            .count()
            .min(u32::MAX as usize) as u32
    }

    fn distinct_predecessors(&self, word: u16) -> u32 {
        self.bigrams
            .iter()
            .filter(|entry| !entry.is_empty() && entry.w1 == word)
            .count()
            .min(u32::MAX as usize) as u32
    }
