...
/// Random-hyperplane SimHash cosine estimate `cos(pi * d_H / 64)` in
/// signed Q16. A distance threshold is still an empirical calibration choice.
pub fn simhash_cosine_q16(a: u64, b: u64) -> i32 {
    const PI_Q16: i128 = 205_887;
    let distance = hamming(a, b).min(64) as i128;
    let (reduced, sign) = if distance > 32 {
        (64 - distance, -1i128)
    } else {
        (distance, 1i128)
    };
    let x_q16 = PI_Q16 * reduced / 64;
    let x2_q16 = (x_q16 * x_q16) >> 16;
    let x4_q16 = (x2_q16 * x2_q16) >> 16;
    let x6_q16 = (x4_q16 * x2_q16) >> 16;
    let cosine = (1i128 << 16) - x2_q16 / 2 + x4_q16 / 24 - x6_q16 / 720;
    (sign * cosine).clamp(i32::MIN as i128, i32::MAX as i128) as i32
}
...
struct BandTable {
    ...
    overflowed: [bool; BUCKETS_PER_BAND],
}
...
impl BandTable {
    ...
    const fn new() -> Self {
        Self {
            ...
            overflowed: [false; BUCKETS_PER_BAND],
        }
    }
...
impl LshIndex {
    ...
    pub fn query(&self, fingerprint: u64, max_hamming: u32, out: &mut [(u64, u32)]) -> usize {
        ...
        let mut exact_fallback_required = false;

        let mut b = 0;
        while b < NUM_BANDS {
            let band_val = ((fingerprint >> (b * BITS_PER_BAND)) & 0xFF) as usize;
            let count = self.bands[b].counts[band_val] as usize;
            exact_fallback_required |= self.bands[b].overflowed[band_val];
            let mut j = 0;
            while j < count {
        ...
        if exact_fallback_required {
            let mut entity = 0usize;
            while entity < self.entity_count && result_count < out.len() {
                let byte_index = entity / 8;
                let bit_index = entity % 8;
                if (seen[byte_index] & (1 << bit_index)) == 0 {
                    let distance = hamming(fingerprint, self.entities[entity].fingerprint);
                    if distance <= max_hamming {
                        out[result_count] = (self.entities[entity].external_id, distance);
                        result_count += 1;
                    }
                }
                entity += 1;
