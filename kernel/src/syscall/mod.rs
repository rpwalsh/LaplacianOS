//! Syscall ABI and dispatch — x86_64.
//!
//! LaplacianOS uses `syscall`/`sysret` (MSR-based fast system calls) on x86_64.
...
fn user_page_allows(cr3: u64, vaddr: u64, writable: bool) -> bool {
    use crate::arch::paging::{flags, pd_index, pdpt_index, pml4_index, pt_index};

    const FRAME_MASK: u64 = crate::arch::paging::FRAME_MASK;
    unsafe {
...
/// `args` — up to 6 arguments [rdi, rsi, rdx, r10, r8, r9].
///
...
pub fn dispatch(nr: u64, args: &[u64; 6]) -> u64 {
    ...
    static TABLE: &[(u64, Fn)] = &[
        ...
        (numbers::SYS_GRAPH_TYPE_PAIR_ADAPT, |_| {
            sys_graph_type_pair_adapt()
        }),
        (numbers::SYS_GRAPH_TYPE_PAIR_STATS, |a| {
            sys_graph_type_pair_stats(a[0], a[1])
        }),
        ...
        // GPU resource management (0x500–0x508)
...
fn sys_login(username_ptr: u64, password_ptr: u64) -> u64 {
    ...
    // Use cstr fallback — lengths are wired in dispatch below.
...
/// Adapt type-pair decay rates from accumulated observations.
/// Only uid=0 tasks may call this.
fn sys_graph_type_pair_adapt() -> u64 {
    let current = crate::sched::current_index();
    let (uid, _) = crate::task::table::task_identity_at(current);
    if uid != 0 {
        return SYSCALL_ERROR;
    }
    crate::graph::temporal::heuristic_type_pair_adaptation() as u64
}

/// Read transition statistics for a single type pair.
/// arg0 = src_kind (u16), arg1 = dst_kind (u16).
/// Returns the transition count for the requested pair.
fn sys_graph_type_pair_stats(src_kind: u64, dst_kind: u64) -> u64 {
    use crate::graph::types::NodeKind;
    ...
    if crate::graph::temporal::snapshot_stats_pair(src_kind as u16, dst_kind as u16, &mut buf) == 0
    {
        return SYSCALL_ERROR;
    }
    buf[0].transitions
...
fn sys_getrandom(buf_ptr: u64, buf_len: u64) -> u64 {
    ...
    let mut off = 0usize;
    while off < out.len() {
        let Some(random) = crate::arch::entropy::random_u64() else {
            out.fill(0);
            return SYSCALL_ERROR;
        };
        let word = random.to_le_bytes();
...
fn sys_cognitive_query(query_ptr: u64, query_len: u64, fingerprint: u64) -> u64 {
    ...
    let phase = (result.channels_used as u64) & 0xFF;
    let evidence = ((result.evidence_count as u64) & 0xFF) << 8;
    let support = ((result.support_score >> 8) as u64 & 0xFFFF) << 16;
    let strategy = ((result.strategy as u64) & 0xFF) << 32;
    phase | evidence | support | strategy
}
...
// ── Phase J GPU compositor syscalls ──────────────────────────────────────────
...
fn sys_expose_toggle(window_w_raw: u64, window_h_raw: u64) -> u64 {
    ...
    // Return 1 if Exposé is now active, 0 if now inactive.
...
fn sys_fetch_update(url_ptr: u64, url_len: u64) -> u64 {
    ...
    // is ed25519-signed — defence in depth requires transport encryption.
    if !crate::net::tls::is_available() {
        serial::write_line(
    ...
    // Static staging buffer — 32 MiB max bundle size.
...
// ====================================================================
// GPU resource syscalls (0x500–0x504)
// ====================================================================
...
/// SYS_COMPOSITOR_CLAIM_DISPLAY — hand scanout ownership to the compositor and
...
/// SYS_GPU_RESOURCE_CREATE — create a GPU resource.
...
/// SYS_GPU_RESOURCE_DESTROY — free a GPU texture resource.
fn sys_gpu_resource_destroy(resource_id: u32) -> u64 {
    if !is_compositor_caller() {
        serial::write_line(b"[syscall] gpu_resource_destroy: EPERM");
        return SYSCALL_ERROR;
    }
    if crate::wm::gpu_exec::free_resource(resource_id) {
        0
    } else {
        serial::write_line(b"[syscall] gpu_resource_destroy: unknown resource or driver failure");
        SYSCALL_ERROR
    }
}
...
/// SYS_GPU_SUBMIT_3D — deprecated legacy entrypoint.
///
/// LaplacianOS-native command submission must use SYS_GPU_SUBMIT.
...
/// SYS_GPU_SURFACE_IMPORT — import a surface backing page into a GPU resource.
///
...
fn sys_gpu_surface_import(surface_id: u32, resource_id: u32) -> u64 {
    ...
    if crate::wm::gpu_exec::import_surface_resource(surface_id, resource_id) {
        0
    } else {
        serial::write_line(b"[syscall] gpu_surface_import: incompatible surface/resource or driver failure");
        SYSCALL_ERROR
    }
}

// ── LaplacianOS-native GPU command submission (0x505–0x508) ───────────────────────
...
/// SYS_GPU_FENCE_ALLOC — allocate a GPU timeline fence.
...
/// SYS_GPU_FENCE_WAIT — block until a fence is signalled.
...
/// SYS_GPU_FENCE_POLL — non-blocking poll of a fence.
...
/// SYS_GPU_SUBMIT — submit a LaplacianOS-native GPU command buffer.
///
