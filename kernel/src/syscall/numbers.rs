...
//! Range 0x300–0x3FF: Graph operations (LaplacianOS-specific).
...
/// LaplacianOS-native submitters must use `SYS_GPU_SUBMIT` instead.
/// Returns 0 on success, u64::MAX on error.
...
/// Submit a LaplacianOS-native GPU command buffer.
...
/// Run bounded type-pair decay-rate adaptation.
/// No arguments. Returns the number of updated type pairs, or u64::MAX.
/// Callable only by uid == 0 tasks.
pub const SYS_GRAPH_TYPE_PAIR_ADAPT: u64 = 0x30D;

/// Read transition statistics for one type pair.
/// arg0 = src_kind (u16), arg1 = dst_kind (u16).
/// Returns the transition count, or u64::MAX on error.
pub const SYS_GRAPH_TYPE_PAIR_STATS: u64 = 0x30E;
