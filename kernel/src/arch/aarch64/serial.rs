...
/// Allocation-free, logging-independent hexadecimal output for exception
/// paths where higher-level mirrors may themselves be the fault source.
pub fn write_hex_raw(val: u64) {
    const HEX: &[u8; 16] = b"0123456789abcdef";
    write_byte(b'0');
    write_byte(b'x');
    for shift in (0..16).rev() {
        write_byte(HEX[((val >> (shift * 4)) & 0xf) as usize]);
    }
}
