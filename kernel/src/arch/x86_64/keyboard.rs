...
pub use crate::input::keyboard::KeyInput;
...
static KEYBOARD: spin::Mutex<KeyboardQueue> = spin::Mutex::new(KeyboardQueue::new());
...
pub fn push_scancode(scancode: u8) {
    interrupts::without_interrupts(|| {
        ...
        if let Some(key) = key {
            drop(queue);
            crate::input::keyboard::push(key);
        }
    });
}

pub fn try_read_key() -> Option<KeyInput> {
    crate::input::keyboard::try_read_key()
}

pub fn has_pending_key() -> bool {
    crate::input::keyboard::has_pending_key()
}
