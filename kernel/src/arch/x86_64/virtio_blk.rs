...
fn find_blk_device() -> Option<PciBlkDevice> {
    ...
    for &did in &[VIRTIO_BLK_DEVICE_MODERN, VIRTIO_BLK_DEVICE_LEGACY] {
        if let Some(info) = crate::arch::pci::find_device(VIRTIO_VENDOR, did) {
            crate::arch::pci::enable_bus_master(info.location);
    ...
    if let Some(info) = crate::arch::pci::find_device(VIRTIO_VENDOR, 0x1001) {
        crate::arch::pci::enable_bus_master(info.location);
        let bar0 = crate::arch::pci::read_u32(
            info.location.bus,
            info.location.slot,
