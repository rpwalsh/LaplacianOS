//! Graph-first telemetry orchestrator.
//!
//! This kernel task fuses telemetry from the twin facade, network stack,
...
fn apply_policy(now_ms: u64) -> (u64, u64, twin::SchedHint, net::NetStats, usize, usize) {
    ...
    let mut quantum = if hint.supported {
        hint.quantum
    } else {
...
fn emit_ai_telemetry(
    ...
    let frame = AiTelemetryFrame {
        ...
        confident: if hint.supported { 1 } else { 0 },
        _pad: 0,
