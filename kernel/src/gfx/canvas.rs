...
// Software global fill path used by the compositor fallback. It writes
// directly to the active scanout framebuffer until full swapchain routing.
...
fn draw_glow_node<T: DrawTarget + ?Sized>(
    ...
    let r = (base_r * depth_scale / 1024).clamp(2, base_r + 4);
    let halo_r = r + r / 2 + 3;
