...
const PROGRAMS: [ProtectedProgram; 24] = [
    ProtectedProgram {
        name: b"init",
...
pub fn spawn_named_service(name: &[u8]) -> Option<TaskId> {
    ...
    if (resolved.task_name == b"launcher"
        || resolved.task_name == b"compositor"
        || resolved.task_name == b"ai-console"
        || resolved.task_name == b"review-ai")
