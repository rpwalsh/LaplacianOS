//! Raw syscall wrappers for ring-3 LaplacianOS applications.
//!
...
pub fn thread_spawn(entry: unsafe extern "C" fn(u64), arg: u64) -> Option<ThreadHandle> {
    ...
    #[cfg(target_arch = "x86_64")]
    let (entry_raw, spawn_arg, initial_stack) = unsafe {
        let ret_slot = (user_stack_top - 8) as *mut u64;
        ret_slot.write_volatile(thread_exit_stub as *const () as u64);
        (entry as *const () as u64, arg, user_stack_top)
    };
    #[cfg(target_arch = "aarch64")]
    let (entry_raw, spawn_arg, initial_stack) = unsafe {
        let start_ptr = (user_stack_top - core::mem::size_of::<ThreadStart>() as u64)
            as *mut ThreadStart;
