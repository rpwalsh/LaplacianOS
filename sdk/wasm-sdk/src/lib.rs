//! LaplacianOS WASM SDK — WASI shim and .gapp bundle tooling.
//!
//! Provides the Rust types that map to the LaplacianOS WASM ABI, helpers for
//! building `.gapp` bundles, and a WASI shim so that code targeting
//! `wasm32-wasip1` can be recompiled for the LaplacianOS WASM sandbox with
//! minimal changes.
//!
//! # Capability flags
...
/// LaplacianOS-WASM host imports exposed to sandboxed modules.
///
...
pub mod host {
    ...
    #[link(wasm_import_module = "laplacianos")]
    unsafe extern "C" {
        /// `laplacianos::write_stdout(ptr: i32, len: i32) -> i32`
        pub fn write_stdout(ptr: i32, len: i32) -> i32;
        /// `laplacianos::channel_send(ch: i32, ptr: i32, len: i32) -> i32`
        pub fn channel_send(ch: i32, ptr: i32, len: i32) -> i32;
        /// `laplacianos::channel_recv(ch: i32, ptr: i32, max: i32) -> i32`
        pub fn channel_recv(ch: i32, ptr: i32, max: i32) -> i32;
        /// `laplacianos::surface_present(surf: i32) -> i32`
        pub fn surface_present(surf: i32) -> i32;
        /// `laplacianos::exit(code: i32) -> !`
