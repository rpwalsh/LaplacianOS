//! LaplacianOS benchmark suite — IPC latency, syscall overhead, VFS throughput.
//!
//! ## Tests
//! - `ipc`      — round-trip latency via laplacianos channel syscalls
...
fn bench_ipc() {
    // Without a running LaplacianOS kernel, measure pipe round-trip as a proxy.
    use std::io::{Read, Write};
...
fn main() {
    ...
    let vfs_path = format!("{}/laplacianos_bench.tmp", tmp);
    ...
    println!("LaplacianOS Benchmark Suite");
    println!("{}", "=".repeat(70));
