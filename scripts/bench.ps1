...
<#
.SYNOPSIS
    LaplacianOS performance benchmark runner.

.DESCRIPTION
    Runs `cargo test -p laplacianos-kernel` in release mode and extracts
...
try {
    ...
    try {
        $raw = & cargo test -p laplacianos-kernel --release -- --nocapture 2>&1
...
if ($exitCode -ne 0) {
    Write-Check 'cargo test -p laplacianos-kernel --release' $false "exit code $exitCode"
    $raw | Select-Object -Last 30 | ForEach-Object { Write-Host "  $_" }
    exit 1
}
Write-Check 'cargo test -p laplacianos-kernel --release' $true
