#!/usr/bin/env pwsh
# LaplacianOS release artifact builder.
#
...
Invoke-Native cargo @("build", "-p", "laplacianos-kernel", "--release", "--target", "x86_64-unknown-none", "-Z", "build-std=core,alloc,compiler_builtins", "--features", "freestanding")

Invoke-Native cargo @("build", "-p", "laplacianos-uefi-loader", "--release", "--target", "x86_64-unknown-uefi", "-Z", "build-std=core,alloc,compiler_builtins", "--features", "uefi-app")

Invoke-Native cargo @("build", "-p", "gpm", "--release")

Invoke-Native cargo @("build", "-p", "laplacianos-appstore", "--release")

$kernelPath = "target\x86_64-unknown-none\release\laplacianos-kernel"
$loaderPath = "target\x86_64-unknown-uefi\release\laplacianos-uefi-loader.efi"
...
$archivePath = Join-Path $RepoRoot "target\laplacianos-release.zip"
