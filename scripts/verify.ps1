#!/usr/bin/env pwsh
# LaplacianOS Full Verification Suite
#
...
$gps = Invoke-Native cargo @("build", "-p", "laplacianos-appstore")
...
if ($SkipBoot) {
    ...
    Copy-Item "target\x86_64-unknown-none\debug\laplacianos-kernel"           "$EspDir\LAPOSK.BIN"
    Copy-Item "target\x86_64-unknown-uefi\debug\laplacianos-uefi-loader.efi"  "$EspDir\EFI\BOOT\BOOTX64.EFI"
    if (Test-Path "target\protected-ring3\laposp.pkg") {
        Copy-Item "target\protected-ring3\laposp.pkg" "$EspDir\LAPOSP.PKG"
    } else {
        Fail "Gate4-8 prebuild: package store" "target\\protected-ring3\\laposp.pkg missing"
    }
    ...
    if ($Ovmf -and $QemuExe) {
        $QemuArgs = @(
            ...
            "-vga", "none",
            "-device", "virtio-gpu-pci,disable-modern=on,disable-legacy=off,xres=1280,yres=800",
        ...
        if (-not (Test-Path $SerialLog)) {
            ...
            $hasPoweroffDispatch = Select-String -Path "kernel\src\syscall\mod.rs" -Pattern "SYS_POWEROFF[^\r\n]*\|_|[\s\S]*crate::acpi::pm::poweroff\(\)" -Quiet
...
if ($Release) {
    ...
    if (Test-Path $hsmAttestation) {
        ...
        if ($attText -match "(?m)^key-id:\s*laplacianos-release-v1\s*$" -and $attText -match "(?m)^hsm-model:\s*\S+" -and $attText -match "(?m)^fingerprint-sha256:\s*\S+" -and $statusOk) {
    ...
    $gpsRelease = Invoke-Native cargo @("build", "-p", "laplacianos-appstore", "--release")
    ...
    $requiredArtifacts = @(
        "target\x86_64-unknown-none\release\laplacianos-kernel",
        "target\x86_64-unknown-uefi\release\laplacianos-uefi-loader.efi",
    ...
    $interop = Invoke-Native cargo @("test", "-p", "laplacianos-tool-sdk", "--test", "ed25519_interop_release_gate")
