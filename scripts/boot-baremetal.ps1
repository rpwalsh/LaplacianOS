#!/usr/bin/env pwsh
# LaplacianOS bare-metal USB prep helper.
#
...
$PackageStore = "target\protected-ring3\laposp.pkg"
...
Copy-Item $KernelElf "$EspDir\LAPOSK.BIN"
Copy-Item $PackageStore "$EspDir\LAPOSP.PKG"
...
if ($UsbRoot) {
    ...
    Copy-Item "$EspDir\LAPOSK.BIN" (Join-Path $usb "LAPOSK.BIN") -Force
    Copy-Item "$EspDir\LAPOSP.PKG" (Join-Path $usb "LAPOSP.PKG") -Force
