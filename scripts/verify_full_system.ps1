#!/usr/bin/env pwsh
# LaplacianOS end-to-end full-system verification.
#
# Vertical slice:
#  1) Boot QEMU
...
function Start-LaplacianOsQemu {
    param(
...
$kernelElf = "target\x86_64-unknown-none\debug\laplacianos-kernel"
$loaderEfi = "target\x86_64-unknown-uefi\debug\laplacianos-uefi-loader.efi"
$pkgStore = "target\protected-ring3\laposp.pkg"

if (-not $NoBuild) {
    Write-Host "== Build ==" -ForegroundColor Cyan
    cargo build -p laplacianos-kernel --target x86_64-unknown-none -Z build-std=core,compiler_builtins,alloc --features freestanding
    if ($LASTEXITCODE -ne 0) { throw "kernel build failed" }
    cargo build -p laplacianos-uefi-loader --target x86_64-unknown-uefi -Z build-std=core,compiler_builtins,alloc --features uefi-app
...
Copy-Item $kernelElf (Join-Path $espDir "LAPOSK.BIN")
Copy-Item $pkgStore (Join-Path $espDir "LAPOSP.PKG")

if (-not (Test-Path $DiskImagePath)) {
    New-SparseRawImage -Path $DiskImagePath -SizeBytes ([int64]$DiskSizeMiB * 1024 * 1024)
}

$marker = "laplacianos-persist-" + [Guid]::NewGuid().ToString("N")
...
try {
    ...
    $proc = Start-LaplacianOsQemu -QemuExe $qemu.Source -Ovmf $ovmf -EspDir (Resolve-Path $espDir).Path -DiskImage (Resolve-Path $DiskImagePath).Path -SerialLog $boot1Log -ForwardPort $resolvedSshForwardPort
    ...
    $proc = Start-LaplacianOsQemu -QemuExe $qemu.Source -Ovmf $ovmf -EspDir (Resolve-Path $espDir).Path -DiskImage (Resolve-Path $DiskImagePath).Path -SerialLog $boot2Log -ForwardPort $resolvedSshForwardPort
