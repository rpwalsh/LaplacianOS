...
$LaplacianosRoot = Split-Path $PSScriptRoot -Parent
$CastleHaleRoot = "C:\Users\react\castlehale_one"

if ([string]::IsNullOrWhiteSpace($ManifestPath)) {
    $ManifestPath = Join-Path $LaplacianosRoot "assets\evals\scce-rollout.json"
}
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = Join-Path $LaplacianosRoot "artifacts\scce-rollout\latest.md"
}
...
function Expand-ManifestPath {
    ...
    $expanded = $PathValue.Replace('$LaplacianosRoot', $LaplacianosRoot)
...
function New-RolloutCorpus {
    ...
    $corpusDir = Join-Path $LaplacianosRoot ("artifacts\scce-rollout\corpus-" + $stamp)
...
if (-not $SkipGuestConfig) {
    $setupScript = Join-Path $LaplacianosRoot "scripts\setup-ai-stack.ps1"
    $setupArgs = @(
        "-ScceRoot", $ScceRoot,
        "-ScceHost", "10.0.2.2",
        "-SccePort", "3000"
    ...
    Write-Step "Configuring SCCE as LaplacianOS modeld's sole inference provider."
...
if (-not $SkipCuratedCorpus) {
    Write-Step "Assembling curated rollout corpus from LaplacianOS and CastleHale sources."
...
$report.Add('Configured guest inference provider: `scce` (no fallback)')
