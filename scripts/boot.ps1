#!/usr/bin/env pwsh
# LaplacianOS Boot Script - builds kernel + loader, creates ESP layout, boots QEMU
#
...
#   - Run from the laplacianos/ workspace root
...
$KERNEL_ELF  = "target\x86_64-unknown-none\debug\laplacianos-kernel"
$LOADER_EFI  = "target\x86_64-unknown-uefi\debug\laplacianos-uefi-loader.efi"
$PACKAGE_STORE = "target\protected-ring3\laposp.pkg"
$ESP_ROOT    = "target\esp"
...
if (-not $NoBuild) {
    ...
    if ($PipeFriendly) {
        & cargo build -p laplacianos-kernel --target x86_64-unknown-none "-Zbuild-std=core,compiler_builtins,alloc" --features freestanding
    } else {
        & cargo build -p laplacianos-kernel --target x86_64-unknown-none "-Zbuild-std=core,compiler_builtins,alloc" --features freestanding 2>&1 | Write-Host
    }
    if ($LASTEXITCODE -ne 0) { Write-Error "Kernel build failed"; exit 1 }

    Write-Host "[boot] Building loader..." -ForegroundColor Yellow
    if ($PipeFriendly) {
        & cargo build -p laplacianos-uefi-loader --target x86_64-unknown-uefi "-Zbuild-std=core,compiler_builtins,alloc" --features uefi-app
    } else {
        & cargo build -p laplacianos-uefi-loader --target x86_64-unknown-uefi "-Zbuild-std=core,compiler_builtins,alloc" --features uefi-app 2>&1 | Write-Host
    }
...
#   target/esp/LAPOSK.BIN            <- kernel ELF
...
Copy-Item $KERNEL_ELF "$ESP_DIR\LAPOSK.BIN"
Copy-Item $PACKAGE_STORE "$ESP_DIR\LAPOSP.PKG"
...
Write-Host "  LaplacianOS - booting in QEMU" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
try {
    ...
    # Default to LaplacianOS-native submit/scanout path; virgl host GL is opt-in.
    ...
    if ($useGpu3D) {
        ...
        Write-Host "[boot] LaplacianOS-native GPU mode requested (virtio-gpu scanout, no host virgl)" -ForegroundColor Cyan
    }
