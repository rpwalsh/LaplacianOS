//! LaplacianOS Shell3D — Phase J complete implementation.
//!
...
fn render(canvas: &mut Canvas<'_>, scene: &Scene, theme: Theme) {
    ...
    canvas.draw_text(12, 11, b"LaplacianOS // NODE: local.host", p.text, 260);
    canvas.draw_text(
        (WIN_W / 2 - 150) as i32,
        11,
                    ...
                    if !left_down && scene.mouse_down {
                        ...
                        if moved <= 8 {
                            if let Some(i) = scene.hover {
                                ...
                                unsafe {
                                    laplacianos_app_sdk::sys::spawn(path);
                                }
                            }
                            if yi >= dy {
                                ...
                                if di >= 0 && (di as usize) < DOCK_APPS.len() {
                                    unsafe {
                                        laplacianos_app_sdk::sys::spawn(DOCK_APPS[di as usize].2);
                                    }
                                }
                            }
                        }
