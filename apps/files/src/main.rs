//! LaplacianOS Files — Phase J complete implementation.
//!
...
fn vfs_readdir(path: &Path) -> Option<Vec<DirEntry>> {
    ...
    let fd = unsafe { laplacianos_app_sdk::sys::vfs_open(path_bytes, 0x0200) };
    if fd == u64::MAX {
        return None;
    }
    let mut raw = vec![0u8; 4096];
    let raw_len = raw.len();
    let n = laplacianos_app_sdk::sys::vfs_readdir(fd, &mut raw, raw_len);
    laplacianos_app_sdk::sys::vfs_close(fd);
    if n == u64::MAX {
        return None;
    }
...
fn vfs_delete(path: &Path) -> bool {
    laplacianos_app_sdk::sys::vfs_unlink(path.as_bytes()) != u64::MAX
}

fn vfs_rename(from: &Path, to: &Path) -> bool {
    laplacianos_app_sdk::sys::vfs_rename(from.as_bytes(), to.as_bytes()) != u64::MAX
}

fn vfs_mkdir(path: &Path) -> bool {
    laplacianos_app_sdk::sys::vfs_mkdir(path.as_bytes()) != u64::MAX
}

fn vfs_read_preview(path: &Path, buf: &mut [u8]) -> usize {
    let fd = unsafe { laplacianos_app_sdk::sys::vfs_open(path.as_bytes(), 0) };
    if fd == u64::MAX {
        return 0;
    }
    let n = laplacianos_app_sdk::sys::vfs_read(fd, buf, buf.len());
    laplacianos_app_sdk::sys::vfs_close(fd);
    if n == u64::MAX {
        0
    } else {
