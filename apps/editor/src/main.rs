//! LaplacianOS Text Editor — Phase J complete implementation.
//!
...
impl Buffer {
    ...
    fn load(&mut self, path: &[u8]) {
        let fd = unsafe { laplacianos_app_sdk::sys::vfs_open(path, 0) };
        if fd == u64::MAX {
            return;
        }
        let mut buf = vec![0u8; 65536];
        let buf_len = buf.len();
        let n = laplacianos_app_sdk::sys::vfs_read(fd, &mut buf, buf_len);
    ...
    fn save(&mut self) {
        ...
        let fd =
            unsafe { laplacianos_app_sdk::sys::vfs_open(&self.filename[..self.filename_len], 0x0002) };
        if fd == u64::MAX {
            return;
        }
        let data = self.text.to_vec();
        let data_len = data.len();
        laplacianos_app_sdk::sys::vfs_write(fd, &data, data_len);
        laplacianos_app_sdk::sys::vfs_close(fd);
        self.dirty = false;
    }
}
