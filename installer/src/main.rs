...
fn install_files(device: &str, source_dir: &PathBuf) -> anyhow::Result<()> {
    let root = format!("{}2", device);
    fs::create_dir_all("/mnt/laplacianos").ok();
    Command::new("mount")
        .args([&root, "/mnt/laplacianos"])
        .status()?;

    // Copy kernel image.
    let kernel_src = source_dir.join("kernel.elf");
    if kernel_src.exists() {
        fs::copy(&kernel_src, "/mnt/laplacianos/boot/laplacianos.elf")?;
        ok("Installed kernel");
    } else {
    ...
    if initrd_src.exists() {
        fs::copy(&initrd_src, "/mnt/laplacianos/boot/initrd.img")?;
        ok("Installed initrd");
    }

    Command::new("umount").arg("/mnt/laplacianos").status()?;
    Ok(())
}
...
fn enrol_tpm(skip_if_absent: bool) -> anyhow::Result<()> {
    ...
    let status = Command::new("tpm2_create")
        .args([
            ...
            "-u",
            "/tmp/laplacianos_seal.pub",
            "-r",
            "/tmp/laplacianos_seal.priv",
        ])
        .status()?;
    if !status.success() {
    ...
    // ── 5. Make the key persistent at a well-known LaplacianOS handle ─────────
    // LaplacianOS uses NV handle 0x81010001 for the root sealing key.
    let status = Command::new("tpm2_evictcontrol")
        .args(["-C", "o", "-c", "/tmp/laplacianos_seal.ctx", "0x81010001"])
        .status()?;
    if !status.success() {
...
fn main() {
    println!("\x1b[1mLaplacianOS Installer v1.0\x1b[0m");
    println!("========================");
    ...
    heading("Installing LaplacianOS");
    let source_dir = PathBuf::from(env::var("LAPLACIANOS_SOURCE").unwrap_or_else(|_| "/cdrom".into()));
    ...
    println!("\n\x1b[1;32mLaplacianOS installed successfully!\x1b[0m");
