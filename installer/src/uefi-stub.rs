//! UEFI stub application — launches the LaplacianOS installer WASM sandbox.
//!
//! This is a minimal UEFI application that:
//! 1. Sets up a basic GOP (Graphics Output Protocol) framebuffer.
//! 2. Loads `installer.wasm` from the EFI System Partition (`\EFI\LaplacianOS\installer.wasm`).
...
//!    kernel entry point at `\EFI\LaplacianOS\kernel.efi`.
//!
//! ## Build
//! Build as `x86_64-unknown-uefi` or `aarch64-unknown-uefi`:
//! ```
//! cargo build --target x86_64-unknown-uefi -p laplacianos-uefi-stub --release
//! ```
//!
//! ## EFI partition layout assumed
//! ```
//! \EFI\LaplacianOS\
//!   BOOTX64.EFI        <- this binary (renamed from laplacianos-uefi-stub.efi)
...
pub extern "efiapi" fn efi_main(
    ...
    // Print banner.
    let banner = utf16!("LaplacianOS Installer UEFI Stub\r\n");
    ...
    //   2. Open \EFI\LaplacianOS\installer.wasm and \EFI\LaplacianOS\installer.sig.
    //   3. Verify ed25519 signature using the LaplacianOS release public key baked
...
/// LaplacianOS release public key (ed25519, 32 bytes).
