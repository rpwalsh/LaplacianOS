//! LaplacianOS UEFI Stage-1 Loader
//!
//! Responsibilities:
//! 1. Discover GOP framebuffer
//! 2. Read the kernel ELF image (`LAPOSK.BIN`) from the EFI System Partition
...
//! 8. Jump to kernel entry (laplacianos_kmain)
//!
...
/// The boot-information structure passed to laplacianos_kmain.
...
fn efi_main(_image: Handle, mut st: SystemTable<Boot>) -> Status {
    ...
    info!("[laplacianos-loader] LaplacianOS UEFI Loader starting");

    // ---- Step 1: GOP framebuffer ----
    let (fb_addr, fb_width, fb_height, fb_stride, fb_format) = discover_gop(bs);
    info!(
        "[laplacianos-loader] GOP: {}x{} stride={} format={} addr={:#x}",
        fb_width,
        fb_height,
        fb_stride,
    ...
    info!(
        "[laplacianos-loader] Kernel file loaded: {} bytes",
        kernel_data.len()
    );
    ...
    unsafe {
        bs.free_pages(kernel_data.ptr as u64, kernel_data.pages)
            .expect("[laplacianos-loader] Failed to free kernel staging pages");
    }
    info!(
        "[laplacianos-loader] ELF loaded: entry={:#x} phys=[{:#x}..{:#x})",
    ...
    info!(
        "[laplacianos-loader] Package store staged: {} bytes v{} entries={} checksum={:#x}",
        package_store.len(),
    ...
    info!("[laplacianos-loader] RSDP addr: {:#x}", rsdp);
    ...
    .expect("[laplacianos-loader] Failed to allocate BootInfo page at 0x90000");
    ...
    .expect("[laplacianos-loader] Failed to allocate REGIONS pages at 0x91000");
    info!(
        "[laplacianos-loader] BootInfo at {:#x}, REGIONS at {:#x}",
        BOOTINFO_PHYS, REGIONS_PHYS
    );

    // ---- Step 5: Memory map + exit boot services ----
    info!("[laplacianos-loader] Preparing to exit boot services");
...
fn load_kernel_from_esp(bs: &BootServices) -> KernelFileBuffer {
    read_file_from_esp(bs, &KERNEL_FILENAME, "LAPOSK.BIN", u64::MAX)
}
...
fn inspect_package_store(data: &[u8]) -> PackageStoreInfo {
    if data.len() < PACKAGE_STORE_HEADER_SIZE || &data[..8] != PACKAGE_STORE_MAGIC {
        panic!("[laplacianos-loader] LAPOSP.PKG has invalid magic");
    }

    let version = u32::from_le_bytes(
        data[8..12]
            .try_into()
            .expect("[laplacianos-loader] package version field"),
    );
    if version != PACKAGE_STORE_VERSION {
        panic!("[laplacianos-loader] LAPOSP.PKG has unsupported version");
    }
    let entry_count = u32::from_le_bytes(
        data[12..16]
            .try_into()
            .expect("[laplacianos-loader] package entry count field"),
    );
    let image_size = u64::from_le_bytes(
        data[16..24]
            .try_into()
            .expect("[laplacianos-loader] package size field"),
    );
    let checksum = u64::from_le_bytes(
        data[24..32]
            .try_into()
            .expect("[laplacianos-loader] package checksum field"),
    );
    if image_size as usize != data.len() {
        panic!("[laplacianos-loader] LAPOSP.PKG size header mismatch");
    }
    let table_end = PACKAGE_STORE_HEADER_SIZE
        .checked_add(entry_count as usize * PACKAGE_STORE_ENTRY_SIZE)
        .expect("[laplacianos-loader] package entry table overflow");
    if table_end > data.len() {
        panic!("[laplacianos-loader] LAPOSP.PKG entry table out of bounds");
    }
    if checksum != package_store_checksum(data) {
        panic!("[laplacianos-loader] LAPOSP.PKG checksum mismatch");
    }
...
fn read_file_from_esp(
    ...
    let fs_handle = bs
        ...
        .expect("[laplacianos-loader] No SimpleFileSystem handle — is an ESP present?");
    ...
    let mut sfs = bs
        ...
        .expect("[laplacianos-loader] Failed to open SimpleFileSystem");

    let mut root = sfs
        .open_volume()
        .expect("[laplacianos-loader] Failed to open ESP root volume");
    ...
    let file_handle = root
        ...
        .unwrap_or_else(|_| panic!("[laplacianos-loader] Failed to open {} on ESP", label));

    let mut regular: RegularFile = match file_handle.into_type().unwrap() {
        FileType::Regular(f) => f,
        FileType::Dir(_) => panic!("[laplacianos-loader] {} is a directory, not a file", label),
    };

    let file_info = regular
        .get_boxed_info::<FileInfo>()
        .unwrap_or_else(|_| panic!("[laplacianos-loader] Failed to query {} metadata", label));
    let file_size = file_info.file_size() as usize;
    if file_size == 0 {
        panic!("[laplacianos-loader] {} is empty", label);
    }
    ...
    let staging_phys = bs
        ...
        .unwrap_or_else(|_| {
            panic!(
                "[laplacianos-loader] Failed to allocate staging pages for {}",
                label
            )
        });
    ...
    while filled < file_size {
        let bytes_read = regular
            ...
            .unwrap_or_else(|_| panic!("[laplacianos-loader] Failed to read {}", label));
        if bytes_read == 0 {
            break;
        }
        filled += bytes_read;
    ...
    if filled != file_size {
        panic!("[laplacianos-loader] Short read for {}", label);
    }

    info!("[laplacianos-loader] Read {}: {} bytes", label, filled);
...
fn load_elf_segments(bs: &BootServices, data: &[u8]) -> (u64, u64, u64) {
    ...
    let header = FileHeader64::<Endianness>::parse(data)
        .expect("[laplacianos-loader] Failed to parse ELF64 header");
    let endian = header
        .endian()
        .expect("[laplacianos-loader] Failed to determine ELF endianness");
    ...
    let phdrs = header
        ...
        .expect("[laplacianos-loader] Failed to read ELF program headers");

    info!(
        "[laplacianos-loader] ELF: entry={:#x} phnum={}",
        e_entry,
        phdrs.len()
    );
    ...
    if phys_start == u64::MAX {
        panic!("[laplacianos-loader] No PT_LOAD segments found in kernel ELF");
    }
    ...
    info!(
        "[laplacianos-loader] Kernel extent: [{:#x}..{:#x}), merged PT_LOAD ranges={}",
        phys_start,
        phys_end,
        merged_ranges.len()
    );
    ...
    for (range_start, range_end) in &merged_ranges {
        ...
        info!(
            "[laplacianos-loader]   alloc range [{:#x}..{:#x}) = {} pages",
        ...
        if let Err(ref e) = alloc_result {
            info!(
                "[laplacianos-loader]   alloc at {:#x} failed ({:?}), writing in-place (boot-services fallback)",
                range_start,
                e.status()
            );
        }
    ...
    for phdr in phdrs {
        ...
        info!(
            "[laplacianos-loader]   PT_LOAD: paddr={:#x} filesz={:#x} memsz={:#x}",
...
fn discover_gop(bs: &BootServices) -> (u64, u32, u32, u32, u32) {
    ...
    match mode.pixel_format() {
        ...
        PixelFormat::BltOnly => {
            info!("[laplacianos-loader] GOP mode is BLT-only; continuing without direct framebuffer");
