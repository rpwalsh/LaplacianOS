//! `DesktopScene` — 3D spatial shell built on top of `laplacianos-scene`.
//!
...
//! # Usage
//! ```no_run
//! use laplacianos_compositor::desktop_scene::DesktopScene;
//! use laplacianos_gfx::command::ResourceId;
//!
//! let mut scene = DesktopScene::new(1920, 1080);
//! let node = scene.spawn_window(42, 100.0, 100.0);
//! scene.tick(0.016);
//! let graph = scene.build_render_graph(ResourceId(1), ResourceId(2));
//! ```
...
impl DesktopScene {
    ...
    pub fn hit_test_screen(&mut self, px: f32, py: f32) -> Option<laplacianos_scene::hit::HitRecord> {
