...
//! | `GpuCmdEncoder`  | When GPU path is enabled               | Encodes LaplacianOS-native commands → SYS_GPU_SUBMIT |
//!
...
//!
//! GPU commands are encoded as typed LaplacianOS-native commands and serialized
//! by `laplacianos-gfx::wire` before submission to the kernel via `SYS_GPU_SUBMIT`.
//!
//! ## CPU blitter
//!
...
use laplacianos_gfx::wire;

#[cfg(target_os = "none")]
#[inline(always)]
fn laplacianos_syscall2(nr: u64, a0: u64, a1: u64) -> u64 {
...
/// Submit a LaplacianOS-native byte-wire command buffer.
const SYS_GPU_SUBMIT: u64 = 0x508;

// ── Native GPU command buffer ────────────────────────────────────────────────

/// Encodes LaplacianOS-native GPU commands and serializes them through laplacianos-gfx wire format.
...
impl GfxContext for CpuBlitter {
    ...
    fn present(&mut self) {
        ...
        #[cfg(target_os = "none")]
        {
            let _ = laplacianos_syscall2(0x401, self.surface_id as u64, 0);
        }
    }
...
/// GPU-path GfxContext — encodes LaplacianOS-native commands into `NativeGpuCommandBuf`.
///
