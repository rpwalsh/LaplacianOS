...
#[inline(always)]
fn fence_syscall(nr: u64, fence: FenceId) -> u64 {
...
impl Fence {
    ...
    pub fn wait(&mut self) {
        ...
        let _ = fence_syscall(SYS_GPU_FENCE_WAIT, self.id);
        self.signalled = true;
    }
    ...
    fn poll_kernel(&self) -> bool {
        ...
        let ret = fence_syscall(SYS_GPU_FENCE_POLL, self.id);
        ret == 1
    }
}
...
impl Timeline {
    ...
    pub fn wait_for(&mut self, value: u64) {
        ...
        const SYS_GPU_FENCE_WAIT: u64 = 0x506;
        let _ = fence_syscall(SYS_GPU_FENCE_WAIT, self.fence_id);
