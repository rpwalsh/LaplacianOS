...
/// LaplacianOS GL context — the stateful API surface.
///
/// ## Usage
/// ```no_run
...
/// let prog = gl.create_program("void main() {}", "void main() {}").unwrap();
/// let vbo  = gl.gen_buffer();
/// gl.bind_buffer(BufferTarget::ArrayBuffer, vbo);
/// gl.buffer_data_raw(BufferTarget::ArrayBuffer, &[0_u8; 72], BufferUsage::StaticDraw);
/// gl.use_program(prog);
