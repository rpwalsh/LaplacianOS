...
//!
//! ```no_run
//! use laplacianos_gfx::{Color, GpuContext, GpuDevice, PixelFormat};
//!
//! let device = GpuDevice::open().expect("GPU substrate unavailable");
//! let sc = device
//!     .create_swapchain(PixelFormat::Bgra8Unorm)
//!     .expect("back buffer allocation failed");
...
/// Submit a LaplacianOS-native wire command buffer (arg0 = ptr, arg1 = len).
const SYS_GPU_SUBMIT: u64 = 0x508; // SYS_GPU_SUBMIT

// ── Raw syscall helper ────────────────────────────────────────────────────────

/// Issue a LaplacianOS ring-3 syscall.
///
/// # Safety
