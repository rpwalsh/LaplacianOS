...
use laplacianos_compositor::{CompositorState, ThemeTone};
use laplacianos_gl::compositor::{OffscreenWindowRenderer, WindowSurfaceTarget};
use laplacianos_gl::math::{Vec2, Vec3, Vec4};
use laplacianos_gl::mesh::{StdVarying, Vertex, build_sphere};
use laplacianos_gl::shader::Shader;
use laplacianos_gl::text::{GlyphAtlas, TextAlign, TextStyle, append_text, append_text_aligned, append_text_lines};
use laplacianos_gl::texture::Texture;
use laplacianos_gl::ui::{Rect, UiBatch, UiRenderer, UiVertex};
use laplacianos_gl::{
...
fn create_window_surface_with_trace(
    ...
    let fbo = fbos[0];
    ctx.framebuffer_texture_2d(fbo, laplacianos_gl::Attachment::Color(0), color_texture, 0);

    let status = ctx.check_framebuffer_status(fbo);
    if status != laplacianos_gl::FramebufferStatus::Complete {
...
impl CompositorRenderer {
    ...
    fn populate_window_batches(
        &mut self,
        state: &CompositorState,
        telemetry: laplacianos_compositor::SceneTelemetry,
        palette: Palette,
        frame: u32,
    ) {
    ...
    fn append_shell_overlay(
        ...
        append_text(
            ...
            &self.atlas,
            self.atlas_texture,
            "LAPLACIANOS // IMMERSIVE FABRIC DESKTOP",
            46.0,
            34.0,
            0.93,
            TextStyle {
        ...
        append_text(
            ...
            &self.atlas,
            self.atlas_texture,
            "virtio-gpu scanout  |  laplacianos-gl compositor  |  native ring-3 desktop takeover",
            48.0,
            54.0,
            0.93,
            TextStyle {
...
fn build_command_window(
    ...
    let lines = [
        "laplacianos@fabric:~$ boot graph-shell --immersive",
        "[ok] compositor       laplacianos-gl scene online",
