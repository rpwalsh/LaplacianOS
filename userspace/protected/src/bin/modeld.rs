...
const CONFIG_PATHS: [&[u8]; 3] = [
    ...
    b"/data/etc/laplacianos/modeld.json",
...
fn try_local_command(
    ...
    if matches_local_command(prompt, b"/ai status")
        ...
        || matches_local_command(prompt, b"backend status")
    {
        return Some((Backend::Unavailable, compose_status_reply(route, config, out)));
    }
    ...
    if matches_local_command(prompt, b"/ai help")
        ...
        || matches_local_command(prompt, b"modeld help")
    {
        return Some((Backend::Unavailable, compose_help_reply(config, out)));
    }
...
fn run_backend(
    ...
    if let Some(len) = query_scce(config, prompt, out) {
        ...
        (Backend::Unavailable, compose_unavailable_reply(config, out))
    }
}
...
fn compose_unavailable_reply(config: &ModelConfig, out: &mut [u8]) -> usize {
    let mut len = 0usize;
    append_bytes(out, &mut len, b"SCCE provider unavailable at ");
    ...
    append_bytes(
        out,
        &mut len,
        b". modeld has no synthetic fallback; restore the configured provider and retry.",
    );
    len
}

fn compose_status_reply(route: Route, config: &ModelConfig, out: &mut [u8]) -> usize {
    let mut len = 0usize;
    append_bytes(out, &mut len, b"LaplacianOS modeld rollout status.\n");
    append_bytes(out, &mut len, b"Configured provider=scce");
    append_byte(out, &mut len, b'\n');
    ...
    append_bytes(
        out,
        &mut len,
        b"Guest commands: /ai status, /ai help. Provider overrides are not supported.\n",
    );
    append_runtime_snapshot(out, &mut len);
    len
}

fn compose_help_reply(_config: &ModelConfig, out: &mut [u8]) -> usize {
    let mut len = 0usize;
    append_bytes(out, &mut len, b"LaplacianOS modeld command help.\n");
    append_bytes(
        out,
        &mut len,
    ...
    append_bytes(
        out,
        &mut len,
        b"The configured SCCE endpoint is the only inference provider.\n",
    );
    append_bytes(out, &mut len, b"Configured provider=scce");
    append_byte(out, &mut len, b'\n');
    append_runtime_snapshot(out, &mut len);
...
fn build_augmented_prompt(user_prompt: &[u8], config: &ModelConfig, out: &mut [u8]) -> usize {
    ...
    append_bytes(
        out,
        &mut len,
        b"You are LaplacianOS modeld. Answer as a graph-first operating participant, not a generic chatbot.\n",
    );
    append_bytes(
        out,
        &mut len,
...
fn select_route(_config: &ModelConfig, _requested_model: &[u8]) -> Route {
    Route {
...
fn backend_label(backend: Backend) -> &'static [u8] {
    match backend {
        Backend::Scce => b"scce",
        Backend::Unavailable => b"unavailable",
    }
}
