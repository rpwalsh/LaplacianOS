//! appstored — LaplacianOS application store and OTA update daemon.
//!
//! Responsibilities:
//!  - Periodically fetch the signed package index from the configured registry
//!  - Verify the index signature using the release key (`/data/etc/laplacianos/release.pub`)
...
/// HTTP registry host (4-byte IPv4, overridden by /data/etc/laplacianos/registry).
...
fn load_registry_ip() -> [u8; 4] {
    const CFG_PATH: &[u8] = b"/data/etc/laplacianos/registry";
