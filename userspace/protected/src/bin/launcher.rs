//! launcher — LaplacianOS Shell3D spatial desktop shell.
//!
...
use laplacianos_ui_sdk::tokens::{tokens, Theme};
...
//
// The central scene region is now driven by the real `laplacianos-gl` software
...
mod gl3d {
    ...
    //! `laplacianos-gl` software OpenGL pipeline. Renders the fusion-core sphere
    //! plus one tinted sphere per `AppCard` orbiting on a hemisphere.

    use super::{Scene, N_APPS, SCENE_H, TITLEBAR_H, WIN_W};
    use laplacianos_app_sdk::canvas::Canvas;
    use laplacianos_gl::gl::{Context, DrawMode, IndexType};
    use laplacianos_gl::math::{Mat4, Vec2, Vec3, Vec4};
    use laplacianos_gl::mesh::{build_sphere, StdVarying, Vertex};
    use laplacianos_gl::pipeline::Target;
...
fn render_fast(canvas: &mut Canvas<'_>, scene: &Scene) {
    ...
    canvas.draw_text(16, 12, b"LaplacianOS Shell3D // runtime fallback", p.text, 320);
...
fn render(canvas: &mut Canvas<'_>, scene: &Scene) {
    ...
    canvas.draw_text(16, 12, b"LaplacianOS // /graph // orchestrator-owned desktop", p.text, 420);
    canvas.draw_text((WIN_W / 2 - 178) as i32, 12, LANE_CAPTIONS[lane], 0xFFFFC47A, 356);
    let mut clock = [0u8; 16];
    canvas.draw_text((WIN_W - 110) as i32, 12, format_clock(scene.now_ms, &mut clock), p.text_muted, 96);

    // ── 3. Central 3D graph network (real laplacianos-gl pipeline) ───────
