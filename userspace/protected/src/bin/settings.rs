//! settings - LaplacianOS control center.
//!
...
impl AppState {
    ...
    fn select_tab(&mut self, idx: usize) {
        ...
        self.set_status(match self.selected {
            ...
            _ => b"LaplacianOS ships as a graph-native desktop, not a generic shell.",
        });
    }
...
fn render(win: &mut Window, state: &AppState) {
    ...
    draw_window_frame(&mut win.canvas(), root, b"LaplacianOS Control", THEME);
...
fn render_about(win: &mut Window, area: Rect, state: &AppState) {
    ...
    let rows: [(&[u8], &[u8], u32); 6] = [
        ...
        (b"Renderer", b"laplacianos-gl", pal.primary),
