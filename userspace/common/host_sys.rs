...
const HOST_REPLY_ENDPOINT_ENV: &str = "LAPLACIANOS_HOST_REPLY_ENDPOINT";
const HOST_SERVICE_ENV: &str = "LAPLACIANOS_HOST_SERVICE";
const HOST_BIN_DIR_ENV: &str = "LAPLACIANOS_HOST_BIN_DIR";
...
fn current_service_name() -> Option<String> {
    ...
    Some(stem.strip_prefix("laplacianos-").unwrap_or(&stem).to_string())
}
...
fn normalized_name(name: &[u8]) -> Option<String> {
    ...
    if raw.is_empty() {
        ...
        Some(raw.strip_prefix("laplacianos-").unwrap_or(&raw).to_string())
    }
}

fn logical_binary_name(service: &str) -> String {
    let base = if service.starts_with("laplacianos-") {
        service.to_string()
    } else {
        format!("laplacianos-{service}")
    };
